...
CREATE TABLE "Driver" (
    ...
    active BOOLEAN NOT NULL DEFAULT TRUE,
    "maxActiveOrders" INT NOT NULL DEFAULT 3,
    "createdAt" TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    "updatedAt" TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE "Order" (
    ...
    "fareValueCents" INT NOT NULL, -- Faturamento padrão da corrida
    "storeAuthorizedBonusCents" INT NOT NULL DEFAULT 0, -- Incentivos manuais de risco autorizados pela loja (INT é seguro para corrida individual)
    "distanceMeters" INT NOT NULL,
    "businessDate" DATE NOT NULL, -- Dia contábil de competência
    "allocationDifficulty" BOOLEAN NOT NULL DEFAULT FALSE, -- Flag para alarmar risco de atraso por falta de motoboy
...
CREATE TABLE "StoreIntegration" (
    ...
    "authMode" VARCHAR(30) NOT NULL DEFAULT 'WEBHOOK',
    "baseUrl" TEXT,
    "webhookUrl" TEXT,
...
-- ============================================================================
-- 10. DRIVER ENTERPRISE P2 - REDISTRIBUICAO E COMUNICACAO
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS "OrderAssignmentAudit" (
    ...
    changed_by_driver_id UUID REFERENCES "Driver"(id) ON DELETE SET NULL,
    reason TEXT NOT NULL,
    "createdAt" TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT order_assignment_actor_xor CHECK (num_nonnulls(changed_by_staff_id, changed_by_client_id, changed_by_driver_id) = 1)
);
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public' AND tablename = 'ComplianceDocument'
        AND policyname = 'Isolamento de Tenant - ComplianceDocument'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public' AND tablename = 'DriverConsentAcceptance'
        AND policyname = 'Isolamento de Tenant - DriverConsentAcceptance'
    ) THEN
...
BEGIN
    ...
    LOOP
        EXECUTE format('
            CREATE POLICY "Isolamento de Tenant - %I" ON "%I"
            AS PERMISSIVE FOR ALL
            USING (is_platform_admin() OR operator_id = current_operator_id());
        ', t_name, t_name);
    END LOOP;
