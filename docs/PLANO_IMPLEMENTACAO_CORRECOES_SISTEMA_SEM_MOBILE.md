...
- O job de deploy passou a usar um entrypoint Python único e sem shell; `STATIC_ROOT`, volume e Nginx foram alinhados em `/app/static`, e o warm-up da deny-list deixou de executar DDL fora das migrations canônicas.
- O auditor de baseline passou a funcionar também como script direto; se a aplicação do schema falhar, o job emite automaticamente a auditoria somente leitura nos próprios logs antes de encerrar.
