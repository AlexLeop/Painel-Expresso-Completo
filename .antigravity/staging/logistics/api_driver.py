...
def upload_to_storage(namespace: str, entity_id: UUID, file) -> str:
    ...
    original_name = (
        file.name if getattr(file, "name", None) else f"upload_{int(time.time())}"
    )
...
def _build_order_offer_detail(order: Order) -> Dict:
    ...
    return {
        ...
        "destination": {
            ...
            "last_stop_location": _serialize_point(last_stop.geom)
            if last_stop
            else None,
        },
...
def _get_driver_device(
    driver: Driver, device_identifier: str
) -> Optional[DriverDevice]:
...
def _get_active_driver_orders(driver: Driver, exclude_order_id: Optional[UUID] = None):
    orders = (
        ...
        .order_by("-acceptedAt", "-requestedAt")
    )
    if exclude_order_id:
...
def _driver_reached_capacity(
    driver: Driver, exclude_order_id: Optional[UUID] = None
) -> bool:
    return _get_active_driver_orders(
        driver, exclude_order_id=exclude_order_id
    ).count() >= _driver_active_orders_limit(driver)
...
def _serialize_driver_consent(
    document: ComplianceDocument, acceptance: Optional[DriverConsentAcceptance]
) -> Dict:
...
def list_orders(request):
    ...
    orders = list(
        Order.objects.filter(operator=driver.operator)
        .filter(
    ...
    try:
        rejected_offer_ids = {
            ...
            and r.get(_offer_rejection_key(driver.id, order.id))
        }
    except (redis.exceptions.ConnectionError, redis.exceptions.TimeoutError):
    ...
    return [
        ...
        if not (
            ...
            and str(order.id) in rejected_offer_ids
        )
    ]


@router.get(
    "/orders/{order_id}", response={200: OrderOfferDetailResponse, 403: dict, 404: dict}
)
...
def get_cockpit(request):
    ...
    active_order_payload = (
        _serialize_active_order(active_orders[0]) if active_orders else None
    )
    active_orders_payload = [_serialize_active_order(order) for order in active_orders]

    shift_payload = None
    open_session = (
        DriverShiftSession.objects.filter(
            driver=driver,
            status="OPEN",
        )
        .select_related("scheduleEntry__store", "scheduleEntry__turno")
        .order_by("-checkInAt")
        .first()
    )
    ...
    if schedule:
        start_minutes = (
            schedule.turno.startTime.hour * 60 + schedule.turno.startTime.minute
        )
        ...
        shift_payload = {
            ...
            "checked_in_at": open_session.checkInAt.isoformat()
            if open_session
            else None,
        }

    required_types = set(
        DriverDocumentRequirement.objects.filter(
...
def get_driver_performance(request, period: str = "today"):
    ...
    acceptance_rate = (
        round((accepted / acceptance_denominator) * 100, 2)
...
def get_driver_calendar(
    request, start_date: Optional[date] = None, end_date: Optional[date] = None
):
    driver = _get_authenticated_driver(request)
    ...
    schedules = (
        ...
        .select_related("store", "turno")
        .order_by("date", "turno__startTime")
    )

    reservations = (
        DriverShiftReservation.objects.filter(
            driver=driver,
            date__gte=start,
            date__lte=end,
        )
...
def create_shift_reservation(request, payload: DriverShiftReservationPayload):
    ...
    if ScheduleEntry.objects.filter(
        driver=driver, turno=turno, date=payload.date
    ).exists():
...
def generate_device_token(request):
    ...
    # Store token in Redis with 12 hour expiration
    try:
        r.setex(f"device_token:{token}", 43200, f"{driver.id}:{driver.operator_id}")
    except (redis.exceptions.ConnectionError, redis.exceptions.TimeoutError):
        from django.http import JsonResponse

        return JsonResponse({"error": "Falha ao comunicar com Redis"}, status=503)

    return {"device_token": token, "expires_in": 43200}
...
def accept_order(request, order_id: UUID):
    ...
    try:
        if r.get(f"deny_list:driver:{driver.id}"):
            ...
            return JsonResponse(
                {"error": "Motorista bloqueado (Deny-list)."}, status=403
            )
        if r.get(f"deny_list:operator:{driver.operator_id}"):
            from django.http import JsonResponse

            return JsonResponse(
                {"error": "Operador suspenso financeiramente na base."}, status=403
            )
    except (redis.exceptions.ConnectionError, redis.exceptions.TimeoutError):
        from django.http import JsonResponse

        return JsonResponse(
            {"error": "Fail-closed de segurança. Sistema indisponível."}, status=503
        )
    ...
    try:
        ...
        if not claimed:
            ...
            return JsonResponse(
                {"error": "Ordem já capturada por outro entregador ou inexistente."},
                status=409,
            )
    except (redis.exceptions.ConnectionError, redis.exceptions.TimeoutError):
        from django.http import JsonResponse
    ...
    with transaction.atomic():
        ...
        if order.operator_id != driver.operator_id:
            ...
            return JsonResponse(
                {"error": "Ordem não pertence ao operador do motorista."}, status=403
            )
        ...
        if order.status == Order.OrderStatus.ACCEPTED and order.driver_id == driver.id:
            ...
            return JsonResponse(
                {"error": "A ordem está reservada para outro motorista."}, status=403
            )
            ...
            return JsonResponse(
                {"error": "Ordem não está mais disponível para aceite."}, status=400
            )
            ...
            try:
                ...
                # 2. Geofence Setup
                geo_key = f"geofence:stops:{order.operator_id}"
...
@router.post(
    ...
    response={200: dict, 403: dict, 404: dict, 409: dict, 503: dict},
)
...
@router.post(
    ...
    response={200: OrderSchema, 403: dict, 404: dict, 409: dict},
)
@idempotent(timeout=86400, schema=OrderSchema)
def release_order(request, order_id: UUID, payload: OrderReleasePayload):
    ...
    with transaction.atomic():
        ...
        if _order_belongs_to_grouped_manifest(order):
            return 409, {
                "error": "A ordem pertence a um manifesto agrupado e precisa ser redistribuída pela operação."
            }
...
def start_order(request, order_id: UUID):
    ...
    with tenant_context(driver.operator_id):
        ...
        if order.driver_id != driver.id:
            ...
            return JsonResponse(
                {"error": "Ordem não pertence a este motorista."}, status=403
            )
        ...
        if order.status == Order.OrderStatus.STARTED:
            ...
            return JsonResponse(
                {
                    "error": f"Ordem não pode ser iniciada a partir do status {order.status}."
                },
                status=400,
            )
        else:
...
def arrive_order(request, order_id: UUID):
    ...
    with tenant_context(driver.operator_id):
        ...
        if order.status == Order.OrderStatus.ARRIVED:
            ...
            return JsonResponse(
                {
                    "error": f"Ordem não pode ser sinalizada como chegou a partir do status {order.status}."
                },
                status=400,
            )
        else:
...
def complete_stops_batch(request, items: List[StopBatchCompleteItem]):
    ...
    with transaction.atomic():
        for item in items:
            ...
            stop = (
                Stop.objects.select_for_update()
                .filter(pk=item.stop_id, order__driver=driver)
                .select_related("order")
                .first()
            )
        ...
        for order_id in orders_to_check:
            ...
            if order and order.status != Order.OrderStatus.COMPLETED:
                pending_stops = Stop.objects.filter(
                    order=order, completedAt__isnull=True
                ).count()
                if pending_stops == 0:
                    order.status = Order.OrderStatus.COMPLETED
                    order.completedAt = django_tz.now()
                    order.save()
                    # CRÍTICO: Não deve haver try/except aqui engolindo erros.
                    # Se o settlement falhar, a exception deve subir e causar rollback
                    # do transaction.atomic(), impedindo o pedido de ficar COMPLETED sem repasse.
...
def check_in_shift(request, payload: ShiftCheckInSchema):
    ...
    # CNH Validation (mock for now, assume driver needs an APPROVED document)
    # This requires querying DriverDocument, so we do a quick check.
    # We will assume if they don't have CNH approved, they can't check in.
    from logistics.models import DriverDocument
    cnh_docs = DriverDocument.objects.filter(driver=driver, document_type="CNH", status="APPROVED")
    if not cnh_docs.exists():
        return 403, {"error": "CNH não aprovada ou pendente. Check-in bloqueado."}

    # Geofence Validation
    if payload.lat is not None and payload.lng is not None and store.geom:
        from django.contrib.gis.geos import Point
        driver_point = Point(payload.lng, payload.lat, srid=4326)
        # Check if driver is within 500 meters of the store
        if store.geom.distance(driver_point) * 100000 > 500: # rough approximation for degrees to meters
            return 403, {"error": "Fora do raio de geofencing da loja."}
    ...
    if existing_open_session:
        return 400, {
            "error": "Já existe uma sessão de turno aberta para este motorista."
        }
    ...
    token_meta = {"driver_id": str(driver.id), "operator_id": str(driver.operator_id)}
...
def upload_document(
    ...
    required_types = DriverDocumentRequirement.objects.filter(
        tax_class=driver.tax_classification, is_required=True
    ).values_list("document_type", flat=True)

    submitted_types = DriverDocument.objects.filter(
        driver=driver, status__in=["PENDING_APPROVAL", "APPROVED"]
    ).values_list("document_type", flat=True)
...
@router.post(
    "/stops/{stop_id}/pickup-proof", response={201: dict, 400: dict, 404: dict}
)
def upload_pickup_proof(
    ...
    proof = Proof.objects.create(
        ...
        stage="PICKUP",
        fileUrl=upload_to_storage("proofs", stop.id, file) if file else None,
...
@router.post(
    "/stops/{stop_id}/delivery-proof", response={201: dict, 400: dict, 404: dict}
)
...
def register_driver_device(request, payload: DriverDeviceRegisterPayload):
    ...
    with transaction.atomic():
        device = (
            DriverDevice.objects.select_for_update()
            .filter(
                operator=driver.operator,
                driver=driver,
        ...
        if (
            ...
            and active_devices.count() >= MAX_ACTIVE_DRIVER_DEVICES
        ):
            return 409, {
...
@router.post("/fcm-token", response={200: dict, 404: dict})
def register_fcm_token(request, payload: FcmTokenPayload):
    driver = _get_authenticated_driver(request)

    # Try to find the latest active device, or create a dummy one to store the token
    device = (
        DriverDevice.objects.filter(
            operator=driver.operator, driver=driver, status="ACTIVE"
        )
        .order_by("-lastSeenAt")
        .first()
    )
    ...
    if not device:
        ...
        device = DriverDevice.objects.create(
            operator=driver.operator,
            driver=driver,
            deviceIdentifier="android-" + str(driver.id)[:8],
            platform="ANDROID",
    ...
    return 200, {"message": "Token atualizado com sucesso"}
    ...
    return [
        {
            ...
            "status": device.status,
            "trusted": device.trusted,
            "last_seen_at": device.lastSeenAt.isoformat()
...
@router.get(
    "/communications/threads/{thread_id}/messages",
...
def send_driver_thread_message(
...
@router.post(
    "/compliance/consents/{consent_id}/accept",
    response={200: dict, 404: dict, 409: dict},
)
@idempotent(timeout=86400)
def accept_driver_consent(request, consent_id: UUID):
    ...
    return 200, {
        ...
        "accepted_at": acceptance.acceptedAt.isoformat()
...
@router.post(
    "/compliance/consents/{consent_id}/revoke",
    response={200: dict, 404: dict, 409: dict},
)
@idempotent(timeout=86400)
def revoke_driver_consent(
    request, consent_id: UUID, payload: DriverConsentRevokePayload
):
    driver = _get_authenticated_driver(request)
    acceptance = DriverConsentAcceptance.objects.filter(
        operator=driver.operator,
    ...
    if acceptance.revokedAt is not None:
        return 200, {
            ...
            "revoked_at": acceptance.revokedAt.isoformat()
