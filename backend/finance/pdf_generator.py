...
class InvoicePDFBuilder:
    ...
    def build(self) -> bytes:
        ...
        elements.append(
            Paragraph(
                f"FATURA DE SERVIÇOS LOGÍSTICOS - {self.operator.name}", title_style
            )
        )
        ...
        elements.append(
            Paragraph(
                f"<b>Período:</b> {self.invoice.startDate.strftime('%d/%m/%Y')} até {self.invoice.endDate.strftime('%d/%m/%Y')}",
                normal_style,
            )
        )
        ...
        summary_data = [
            ...
            [
                "Produção Líquida (Entregas)",
                self._format_money(self.invoice.totalNetProducaoCents),
            ],
            [
                "Garantidas (Diárias/Horas)",
                self._format_money(self.invoice.totalNetGarantidaCents),
            ],
            [
                "Taxa Administrativa do Operador",
                self._format_money(self.invoice.administrativeFeeCents),
            ],
            [
                "Taxa de Supervisão/Software",
                self._format_money(self.invoice.supervisionFeeCents),
            ],
            [
                "Débitos Anteriores (Rollover)",
                self._format_money(self.invoice.pendingDebitCarriedCents),
            ],
        ...
        elements.append(
            Paragraph(
                "<b>Detalhamento de Cobranças (Breakdown)</b>", styles["Heading2"]
            )
        )
        elements.append(Spacer(1, 12))
        ...
        for item in self.line_items:
            data_str = (
                item.businessDate.strftime("%d/%m/%Y") if item.businessDate else "N/A"
            )
        ...
        if self.invoice.pixCopyPaste:
            ...
            elements.append(
                Paragraph("<b>Código de Barras do Boleto:</b>", normal_style)
            )
