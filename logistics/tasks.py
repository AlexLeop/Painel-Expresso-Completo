...
from logistics.models import (
    Order, Stop, GeofenceEvent, ComplianceRetentionPolicy,
    DriverConsentAcceptance, DriverDeviceSecurityEvent, DriverIncident,
    DriverOfflineSyncBatch, Proof,
)
...
def _apply_retention_policy(policy: ComplianceRetentionPolicy, cutoff_at, executed_at) -> int:
    processed = 0
    ...
    if policy.resourceType == "DRIVER_CONSENT_ACCEPTANCE":
        ...
        for record in records.iterator():
            ...
            record.save(update_fields=["deviceIdentifier", "ipAddress", "userAgent", "metadata"])
            processed += 1
        return processed
    ...
    if policy.resourceType == "PROOF":
        ...
        for record in records.iterator():
            ...
            record.barcode = None
            record.metadata = updated_metadata
            record.save(update_fields=["deviceIdentifier", "confirmationCode", "qrCode", "barcode", "metadata"])
            processed += 1
        return processed
...
def process_telemetry_batch():
    ...
    while True:
        cursor, keys = r.scan(cursor=cursor, match="analytics:telemetry_batch_*", count=100)
                ...
                if distance_accumulated > 0:
                    ...
                    if operator_id:
                        ...
                        with tenant_context(operator_id):
                            active_manifest = Manifest.objects.filter(
                                operator_id=operator_id, 
                                driver_id=drv_id, 
                                status__in=[Manifest.ManifestStatus.OPEN, Manifest.ManifestStatus.LOCKED]
                            ...
                            if active_manifest:
                                ...
                                if dist_int > 0:
                                    ...
                                    logger.info(f"[TELEMETRY] Driver {drv_id} acumulou +{dist_int}m no Manifest {active_manifest.id}.")
            ...
            try:
                ...
                with tenant_context(order.operator_id):
                    ...
                    if order.status == Order.OrderStatus.STARTED:
                        from django.utils import timezone as django_tz
                        Order.validate_status_transition(order.status, Order.OrderStatus.ARRIVED)
                        updated = Order.objects.filter(pk=order.pk, status=Order.OrderStatus.STARTED).update(
                            status=Order.OrderStatus.ARRIVED,
                            arrivedAt=django_tz.now()
                        )
...
def monitor_expiring_documents():
    ...
    # TODO: Disparar Push Notifications (FCM) para documentos vencendo em 30, 15 e 7 dias.
...
def execute_retention_policies():
    ...
    policies = ComplianceRetentionPolicy.objects.filter(
        active=True,
    ).select_related("operator").order_by("operator_id", "resourceType")
