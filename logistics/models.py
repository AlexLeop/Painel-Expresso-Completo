...
class Client(TimeStampedTenantModel):
    ...
    name = models.CharField(max_length=255, help_text="Nome da franquia ou cliente embarcador.")
...
class Store(TimeStampedTenantModel):
    ...
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    client = models.ForeignKey(Client, on_delete=models.CASCADE, db_column='client_id')
    name = models.CharField(max_length=255, help_text="Identificação única da loja na rede.")
    averagePrepTimeMinutes = models.IntegerField(default=15, db_column='averagePrepTimeMinutes', help_text="Tempo médio de preparo para estimativa do SLA.")
    geom = gis_models.PointField(geography=True, srid=4326, help_text="Geolocalização (Lat/Lng) da loja para cálculos ST_DWithin.")
...
class Vehicle(TenantModel):
    ...
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    plate = models.CharField(max_length=10, unique=True, help_text="Placa ou identificação visual.")
    type = models.CharField(max_length=20, choices=VehicleType.choices, default=VehicleType.MOTORCYCLE)
    active = models.BooleanField(default=True)
...
class Driver(TimeStampedTenantModel):
    ...
    geom = gis_models.PointField(geography=True, srid=4326, null=True, blank=True, help_text="Último ping válido do device.")
    heading = models.IntegerField(default=0, null=True, blank=True)
    speedKmh = models.IntegerField(default=0, null=True, blank=True, db_column='speedKmh')
    lastPingAt = models.DateTimeField(null=True, blank=True, db_column='lastPingAt')
    active = models.BooleanField(default=True)
    operational_status = models.CharField(max_length=30, default='OFFLINE', db_column='operational_status')
    ...
    tax_classification = models.CharField(max_length=50, default='PESSOA_FISICA_AUTONOMO')
...
class Place(TenantModel):
    ...
    accessCode = models.CharField(max_length=50, null=True, blank=True, db_column='accessCode')
...
class ServiceZone(TimeStampedTenantModel):
    ...
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    name = models.CharField(max_length=255)
    geom = gis_models.PolygonField(geography=True, srid=4326, help_text="Polígono delimitador (Geofence).")
...
class ScheduleEntry(TimeStampedTenantModel):
    ...
    minGuaranteedOverrideCents = models.BigIntegerField(null=True, blank=True, db_column='minGuaranteedOverrideCents')
...
class ScheduleEntryAudit(TenantModel):
    ...
    """
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    scheduleEntry = models.ForeignKey(ScheduleEntry, on_delete=models.CASCADE, db_column='scheduleEntryId')
    staff = models.ForeignKey(StaffMember, on_delete=models.RESTRICT, db_column='staffId')
    previousStore = models.ForeignKey(Store, null=True, blank=True, on_delete=models.SET_NULL, db_column='previousStoreId', related_name='audit_previous')
    newStore = models.ForeignKey(Store, null=True, blank=True, on_delete=models.SET_NULL, db_column='newStoreId', related_name='audit_new')
...
class Manifest(TimeStampedTenantModel):
    ...
    status = models.CharField(max_length=20, choices=ManifestStatus.choices, default=ManifestStatus.OPEN)
    groupingMethod = models.CharField(max_length=20, choices=GroupingMethod.choices, default=GroupingMethod.AUTOMATIC, db_column='groupingMethod')
    totalDistanceMeters = models.IntegerField(default=0, db_column='totalDistanceMeters', help_text="Distância acumulada do manifesto.")
    totalEtaSeconds = models.IntegerField(default=0, db_column='totalEtaSeconds', help_text="Tempo estimado total.")
...
class Order(TenantModel):
    ...
    VALID_TRANSITIONS = {
        ...
        OrderStatus.OFFERED: [OrderStatus.ACCEPTED, OrderStatus.CANCELED, OrderStatus.PREPARING],
        # ACCEPTED -> OFFERED cobre devolucao pelo motorista e redistribuicao operacional
        # antes do inicio efetivo da corrida.
        OrderStatus.ACCEPTED: [OrderStatus.STARTED, OrderStatus.CANCELED, OrderStatus.OFFERED],
    ...
    store = models.ForeignKey(Store, on_delete=models.CASCADE, db_column='store_id')
    driver = models.ForeignKey(Driver, on_delete=models.SET_NULL, null=True, blank=True, db_column='driver_id')
    manifest = models.ForeignKey(Manifest, on_delete=models.SET_NULL, null=True, blank=True, db_column='manifest_id')
    status = models.CharField(max_length=30, choices=OrderStatus.choices, default=OrderStatus.PREPARING)
    ...
    storeAuthorizedBonusCents = models.IntegerField(default=0, db_column='storeAuthorizedBonusCents')
    ...
    allocationDifficulty = models.BooleanField(default=False, db_column='allocationDifficulty')
    ...
    external_order_id = models.CharField(max_length=255, null=True, blank=True, db_column='external_order_id')
    external_source = models.CharField(max_length=50, null=True, blank=True, db_column='external_source', help_text="Ex: 'ifood', 'hubster', 'delivery_direto'")
...
class Stop(TenantModel):
    ...
    type = models.CharField(max_length=20, choices=StopType.choices, default=StopType.DROPOFF)
    geom = gis_models.PointField(geography=True, srid=4326)
    requiresPin = models.BooleanField(default=False, db_column='requiresPin')
    deliveryPinHash = models.TextField(null=True, blank=True, db_column='deliveryPinHash')
    completedAt = models.DateTimeField(null=True, blank=True, db_column='completedAt')
...
class ManifestStop(TenantModel):
    ...
    """
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    manifest = models.ForeignKey(Manifest, on_delete=models.CASCADE, db_column='manifest_id')
...
class Proof(TenantModel):
    ...
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    stop = models.ForeignKey(Stop, on_delete=models.CASCADE, db_column='stop_id')
    type = models.CharField(max_length=20, choices=ProofType.choices, default=ProofType.PHOTO)
    ...
    gpsAccuracyMeters = models.IntegerField(null=True, blank=True, db_column='gpsAccuracyMeters')
    deviceIdentifier = models.CharField(max_length=255, null=True, blank=True, db_column='deviceIdentifier')
    confirmationCode = models.CharField(max_length=100, null=True, blank=True, db_column='confirmationCode')
...
class DriverShiftSession(TenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    driver = models.ForeignKey(Driver, on_delete=models.CASCADE, db_column='driver_id')
    scheduleEntry = models.ForeignKey(ScheduleEntry, null=True, blank=True, on_delete=models.SET_NULL, db_column='scheduleEntryId')
...
class DriverIncident(TenantModel):
    ...
    stop = models.ForeignKey(Stop, null=True, blank=True, on_delete=models.SET_NULL, db_column='stop_id')
...
class DriverIncidentAttachment(TenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    incident = models.ForeignKey(DriverIncident, on_delete=models.CASCADE, db_column='incident_id')
...
class DriverDeviceSecurityEvent(TenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    driver = models.ForeignKey(Driver, on_delete=models.CASCADE, db_column='driver_id')
    device = models.ForeignKey(DriverDevice, on_delete=models.CASCADE, db_column='device_id')
...
class OrderAssignmentAudit(TenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    order = models.ForeignKey(Order, on_delete=models.CASCADE, db_column='order_id')
    previousDriver = models.ForeignKey(Driver, null=True, blank=True, on_delete=models.SET_NULL, related_name='assignment_audits_previous', db_column='previous_driver_id')
    newDriver = models.ForeignKey(Driver, null=True, blank=True, on_delete=models.SET_NULL, related_name='assignment_audits_new', db_column='new_driver_id')
    changedByStaff = models.ForeignKey(StaffMember, null=True, blank=True, on_delete=models.SET_NULL, db_column='changed_by_staff_id')
    changedByClient = models.ForeignKey(ClientPortalUser, null=True, blank=True, on_delete=models.SET_NULL, db_column='changed_by_client_id')
    changedByDriver = models.ForeignKey(Driver, null=True, blank=True, on_delete=models.SET_NULL, related_name='assignment_audits_actor', db_column='changed_by_driver_id')
...
class DriverCommunicationThread(TimeStampedTenantModel):
    ...
    driver = models.ForeignKey(Driver, on_delete=models.CASCADE, db_column='driver_id')
    sourceType = models.CharField(max_length=20, db_column='sourceType', default='STORE')
    status = models.CharField(max_length=20, default='OPEN')
...
class DriverCommunicationMessage(TenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    thread = models.ForeignKey(DriverCommunicationThread, on_delete=models.CASCADE, db_column='thread_id')
    senderType = models.CharField(max_length=20, db_column='senderType')
    senderName = models.CharField(max_length=255, null=True, blank=True, db_column='senderName')
...
class ComplianceDocument(TimeStampedTenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    audienceType = models.CharField(max_length=20, db_column='audienceType', default='DRIVER')
...
class DriverConsentAcceptance(TenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    driver = models.ForeignKey(Driver, on_delete=models.CASCADE, db_column='driver_id')
    document = models.ForeignKey(ComplianceDocument, on_delete=models.CASCADE, db_column='document_id')
    deviceIdentifier = models.CharField(max_length=255, null=True, blank=True, db_column='deviceIdentifier')
    ipAddress = models.CharField(max_length=64, null=True, blank=True, db_column='ipAddress')
...
class PrivacyDataRequest(TimeStampedTenantModel):
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    subjectType = models.CharField(max_length=30, db_column='subjectType', default='DRIVER')
    driver = models.ForeignKey(Driver, null=True, blank=True, on_delete=models.CASCADE, db_column='driver_id')
    clientPortalUser = models.ForeignKey(ClientPortalUser, null=True, blank=True, on_delete=models.CASCADE, db_column='client_portal_user_id')
...
class ComplianceRetentionPolicy(TimeStampedTenantModel):
    ...
    active = models.BooleanField(default=True)
    lastExecutedAt = models.DateTimeField(null=True, blank=True, db_column='lastExecutedAt')
    metadata = models.JSONField(default=dict, blank=True)
