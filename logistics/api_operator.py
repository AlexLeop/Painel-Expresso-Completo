...
def _serialize_privacy_request(data_request: PrivacyDataRequest) -> Dict:
    return {
        ...
        "resolved_at": data_request.resolvedAt.isoformat()
        if data_request.resolvedAt
        else None,
        "metadata": data_request.metadata or {},
        "created_at": data_request.createdAt.isoformat()
        if data_request.createdAt
        else None,
        "updated_at": data_request.updatedAt.isoformat()
        if data_request.updatedAt
        else None,
    }
...
def _serialize_retention_policy(policy: ComplianceRetentionPolicy) -> Dict:
    return {
        ...
        "last_executed_at": policy.lastExecutedAt.isoformat()
...
def get_dashboard(request):
    ...
    orders_today = Order.objects.filter(
        operator=staff.operator, businessDate=today
    ).count()
...
def create_schedule(request, payload: SchedulePayload):
    ...
    with transaction.atomic():
        ...
        schedule, created = ScheduleEntry.objects.get_or_create(
            ...
            defaults={"minGuaranteedOverrideCents": payload.minGuaranteedOverrideCents},
        )
...
def dispatch_order(request, payload: DispatchPayload):
    ...
    with transaction.atomic():
        ...
        b_date = resolve_store_business_date(
            store, explicit_business_date=payload.businessDate
        )
...
def reassign_order(request, order_id: UUID, payload: OrderReassignmentPayload):
    ...
    if payload.new_driver_id:
        new_driver = Driver.objects.filter(
            pk=payload.new_driver_id, operator=staff.operator
        ).first()
        if not new_driver:
...
def send_operator_thread_message(
    ...
    return [
        {
            ...
            "effective_at": document.effectiveAt.isoformat()
            if document.effectiveAt
            else None,
            "archived_at": document.archivedAt.isoformat()
...
@router.post(
    "/compliance/documents/{document_id}/archive",
...
def list_privacy_requests(request, status: str = "", subject_type: str = ""):
    staff = require_role(["ADMIN", "MANAGER", "OPERATOR_ROLE", "VIEWER"])(request)
    queryset = (
        PrivacyDataRequest.objects.filter(
            operator=staff.operator,
        )
        .select_related("driver", "clientPortalUser")
        .order_by("-createdAt")
    )
...
@router.post(
    "/privacy/requests/{request_id}/resolve",
    response={200: dict, 400: dict, 404: dict, 409: dict},
)
def resolve_privacy_request(
    request, request_id: UUID, payload: PrivacyDataRequestResolvePayload
):
    staff = require_role(["ADMIN", "MANAGER", "OPERATOR_ROLE"])(request)
    ...
    if normalized_status not in SUPPORTED_PRIVACY_REQUEST_STATUSES - {
        "OPEN",
        "IN_PROGRESS",
    }:
        return 400, {
    ...
    data_request.save(
        update_fields=["status", "resolution", "resolvedAt", "metadata", "updatedAt"]
    )
...
@router.get(
    "/compliance/retention-policies", response=List[ComplianceRetentionPolicyItem]
)
...
@router.post(
    "/compliance/retention-policies", response={200: dict, 201: dict, 400: dict}
)
...
@router.post(
    "/compliance/retention-policies/{policy_id}/disable",
    response={200: dict, 404: dict, 409: dict},
)
...
def get_proof_url(request, proof_id: UUID):
    ...
    try:
        ...
        resp = httpx.post(
            sign_url, json={"expiresIn": 900}, headers=headers, timeout=5.0
        )
