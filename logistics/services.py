...
class TSPEngine:
    ...
    def sequence_stops(store: Store, stops: List[Stop]) -> List[Stop]:
        ...
        while unsequenced:
            ...
            nearest = Stop.objects.filter(id__in=ids).annotate(
                knn_dist=RawSQL("geom <-> ST_SetSRID(ST_MakePoint(%s, %s), 4326)", (current_geom.x, current_geom.y))
            ).order_by('knn_dist').first()
    ...
    def build_manifest(operator_id, store: Store, stops: List[Stop]) -> Manifest:
        ...
        manifest = Manifest.objects.create(
            ...
            status=Manifest.ManifestStatus.OPEN
        )
