...
from finance.models import Wallet, WalletTransaction, WithdrawalRequest, ManualEntry, WeeklyStoreInvoice
from logistics.models import Driver, Store, Client
from accounts.auth import require_role
from config.idempotency import idempotent

from shared_schemas.finance import (
    WalletResponse, WithdrawalRequestPayload, WithdrawalResponse, 
    ManualEntryPayload, ManualEntryResponse, InvoiceResponse, TransactionResponse
)
...
def list_invoices(request):
    ...
    return list(WeeklyStoreInvoice.objects.filter(operator=staff.operator).order_by('-createdAt'))
...
def get_gerencial_data(request):
    ...
    semana_atual = WeeklyStoreInvoice.objects.filter(
        operator=operator,
    ...
    snapshots_query = WeeklyStoreInvoice.objects.filter(
        operator=operator,
        status=WeeklyStoreInvoice.InvoiceStatus.FINALIZED
    ).select_related('store').order_by('-startDate')[:50]
    snapshots = []
    ...
    for invoice in snapshots_query:
        ...
        snapshots.append(GerencialSnapshotResponse(
