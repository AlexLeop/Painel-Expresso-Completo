...
from finance.models import (
    Contract, Wallet, OperatorInternalWallet, WalletTransaction, WeeklyInvoiceLineItem, WeeklyStoreInvoice
)
...
class SettlementEngine:
    ...
    def settle_order(order: Order):
        ...
        with transaction.atomic():
            ...
            try:
                contract = Contract.objects.get(store=order.store, operator=order.operator)
            except Contract.DoesNotExist:
                logger.error(f"FATAL: Contrato não encontrado para a loja {order.store.id}")
                raise ValueError("Loja sem contrato ativo. Não é possível liquidar a corrida.")
            ...
            if contract.compensationMode != Contract.CompensationMode.PRODUCAO:
                ...
                logger.info("Order %s não gera repasse imediato (Modo Garantida).", order.id)
                return
            ...
            if order.driver:
                ...
                logger.info(f"Creditados {driver_fee} cents na wallet do motorista {order.driver.id}.")
            ...
            store_cost = order.fareValueCents if order.fareValueCents else contract.rideFeePerDeliveryCents
            ...
            logger.info(f"Adicionado {store_cost} cents na fatura da loja {order.store.id}.")
