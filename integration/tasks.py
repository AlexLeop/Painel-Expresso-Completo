...
def flush_outbox_events(self):
    ...
    for operator_id in operators:
        ...
        with tenant_context(operator_id):
            events = list(IntegrationOutbox.objects.select_for_update(skip_locked=True).filter(
                status__in=[IntegrationOutbox.OutboxStatus.PENDING, IntegrationOutbox.OutboxStatus.FAILED]
            )[:50])
            ...
            try:
                ...
                with tenant_context(operator_id):
                    if event.aggregateType == 'ORDER':
                        ...
                        latest_delivery_proof = Proof.objects.filter(
                            operator_id=operator_id,
                            stop__order=order,
                            stage='DELIVERY',
                        ).select_related('stop').order_by('-capturedAt').first()
                    ...
                    try:
                        if r.get(f"deny_list:driver:{driver_id}"):
                            with tenant_context(operator_id):
                                ...
                                event.failReason = "BLOCKED_MID_FLIGHT: Driver na Deny-list."
                ...
                with tenant_context(operator_id):
                    integration = StoreIntegration.objects.filter(store=store, active=True).first()
                    if not integration:
                    ...
                    outbound_payload = build_outbound_event_payload(
                        ...
                        delivery_proof=latest_delivery_proof if 'latest_delivery_proof' in locals() else None,
                    ...
                    if provider == "DELIVERY_DIRETO":
                        ...
                        if not secret:
                            raise Exception("Segredo de integração ausente para DELIVERY_DIRETO.")
                        body_bytes = requests.models.complexjson.dumps(outbound_payload).encode("utf-8")
                ...
                if should_skip_outbound_partner_status(integration, event.eventType):
                    response = None
                elif uses_open_delivery_polling(integration) and provider in {"DELIVERY_DIRETO", "99FOOD"}:
                    response = send_open_delivery_order_status(integration, order, event.eventType)
                    if response is None:
                        raise Exception(f"Evento {event.eventType} ainda não mapeado para Open Delivery.")
                else:
                    if not webhook_url:
                        raise Exception("A URL do Webhook na StoreIntegration está vazia.")
                ...
                if response is None or response.status_code in [200, 201, 202, 204]:
                    with tenant_context(operator_id):
                        IntegrationEventAudit.objects.create(
                            ...
                            externalOrderId=order.external_order_id if order and getattr(order, "external_order_id", None) else None,
                            merchantReference=integration.merchantId,
                            deliveryStatus="NOT_REQUIRED" if response is None else "ACKNOWLEDGED",
                            httpStatusCode=None if response is None else response.status_code,
                            payload=outbound_payload,
                            responsePayload={"text": None if response is None else response.text[:2000]},
                ...
                with tenant_context(operator_id):
                    if 'store' in locals() and store:
                        IntegrationEventAudit.objects.create(
                            operator_id=operator_id,
                            store=store,
                            order=order if 'order' in locals() and event.aggregateType == 'ORDER' else None,
                            provider=provider if 'provider' in locals() else 'UNKNOWN',
                            direction="OUTBOUND",
                            eventType=event.eventType,
                            externalEventId=str(event.id),
                            externalOrderId=order.external_order_id if 'order' in locals() and order and getattr(order, "external_order_id", None) else None,
                            merchantReference=integration.merchantId if 'integration' in locals() and integration else None,
                            deliveryStatus="FAILED",
                            payload=outbound_payload if 'outbound_payload' in locals() else event.payload,
                            failReason=str(e),
                            processedAt=timezone.now(),
                        )
                    event.status = IntegrationOutbox.OutboxStatus.FAILED
...
def poll_partner_events(self):
    ...
    for integration in integrations:
        ...
        with tenant_context(operator_id):
            try:
                if uses_open_delivery_polling(integration) and integration.provider in {"DELIVERY_DIRETO", "99FOOD"}:
                    events = poll_open_delivery_events(integration)
                elif uses_ifood_mercado_polling(integration) and integration.provider == "IFOOD":
                ...
                for polled_event in events:
                    ...
                    audit = IntegrationEventAudit.objects.create(
                        ...
                        direction="INBOUND",
                        eventType=polled_event.get("code") or polled_event.get("eventType") or "POLLING_EVENT",
    ...
    for operator_id in operators:
        ...
        for event in processing_events:
            try:
                ...
                payload = envelope.get("raw_payload") or envelope.get("body") or envelope
                # Aqui entra o Adapter Pattern de cada PDV.
                source = normalize_provider(event.eventType.replace('_WEBHOOK_RECEIVED', ''))
                ...
                with tenant_context(operator_id):
                    ...
                    store = StoreIntegration.objects.select_related("store").get(store_id=store_id).store
                    resolved_business_date = resolve_store_business_date(
                        store,
                        explicit_business_date=business_dt.date() if business_dt else None,
                    ...
                    if not created and should_apply_inbound_status(order.status, mapped_status):
                        try:
                    ...
                    if audit_id:
                        IntegrationEventAudit.objects.filter(
                            ...
                            responsePayload={
                                ...
                                "business_date": order.businessDate.isoformat() if order.businessDate else None,
                            },
                        )
