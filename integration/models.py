...
class StoreIntegration(TimeStampedTenantModel):
    ...
    provider = models.CharField(max_length=50, help_text="Provedor (Ex: HUBSTER, IFOOD).")
    clientId = models.CharField(max_length=255, null=True, blank=True, db_column='clientId')
    clientSecret = models.TextField(null=True, blank=True, db_column='clientSecret', help_text="Senha de integração.")
    merchantId = models.CharField(max_length=100, null=True, blank=True, db_column='merchantId')
    ...
    def _is_fernet_token(self, value: str) -> bool:
        ...
        try:
            ...
            return len(decoded) >= 57  # Header(1) + Timestamp(8) + IV(16) + min_block(16) + HMAC(32)
        except Exception:
            return False
    ...
    def set_client_secret(self, raw_secret: str):
        ...
        if raw_secret:
            self.clientSecret = get_cipher().encrypt(raw_secret.encode('utf-8')).decode('utf-8')
        else:
    ...
    def get_client_secret(self) -> str:
        ...
        if self.clientSecret:
            try:
                return get_cipher().decrypt(self.clientSecret.encode('utf-8')).decode('utf-8')
            except Exception:
                return None
        return None
...
class IntegrationOutbox(TimeStampedTenantModel):
    ...
    operator = models.ForeignKey(Operator, on_delete=models.CASCADE, db_column='operator_id')
    aggregateType = models.CharField(max_length=100, db_column='aggregateType', help_text="Entidade alvo (Ex: ORDER).")
    aggregateId = models.UUIDField(db_column='aggregateId', help_text="ID do registro afetado.")
    sequenceNumber = models.BigIntegerField(unique=True, editable=False, db_column='sequenceNumber', help_text="Garante FIFO absoluto. Auto-gerado via BIGSERIAL no Postgres.")
    eventType = models.CharField(max_length=100, db_column='eventType', help_text="Ação ocorrida (Ex: ORDER_ACCEPTED).")
    payload = models.JSONField(help_text="Snapshot do payload JSON a ser transmitido ao webhook.")
    status = models.CharField(max_length=50, choices=OutboxStatus.choices, default=OutboxStatus.PENDING)
    attempts = models.IntegerField(default=0, help_text="Retries com backoff exponencial.")
    lastAttemptAt = models.DateTimeField(null=True, blank=True, db_column='lastAttemptAt')
    failReason = models.TextField(null=True, blank=True, db_column='failReason', help_text="Armazena os status codes ou timeouts (504, 500, etc).")
...
class IntegrationEventAudit(TimeStampedTenantModel):
    ...
    externalEventId = models.CharField(max_length=255, null=True, blank=True, db_column='externalEventId')
    externalOrderId = models.CharField(max_length=255, null=True, blank=True, db_column='externalOrderId')
    merchantReference = models.CharField(max_length=100, null=True, blank=True, db_column='merchantReference')
    deliveryStatus = models.CharField(max_length=30, db_column='deliveryStatus')
    httpStatusCode = models.IntegerField(null=True, blank=True, db_column='httpStatusCode')
    payload = models.JSONField(default=dict, blank=True)
    responsePayload = models.JSONField(default=dict, blank=True, db_column='responsePayload')
