...
class TestIdempotency:
    ...
    def test_missing_idempotency_key(self, client):
        ...
        assert "Idempotency-Key" in data.get('error', '') or "Idempotency-Key" in data.get('message', '')
...
def test_idempotency_accepts_shared_redis_fallback_flag(monkeypatch):
    ...
    assert module._env_flag("IDEMPOTENCY_ALLOW_MEMORY_FALLBACK", "REDIS_ALLOW_MEMORY_FALLBACK") is True
...
def test_memory_redis_supports_sorted_set_token_invalidation_flow():
    ...
    redis_client.zadd("fastlane:driver_tokens:driver-1", {"token-b": 20, "token-a": 10, "token-c": 30})

    assert redis_client.zrange("fastlane:driver_tokens:driver-1", 0, -1) == ["token-a", "token-b", "token-c"]

    removed = redis_client.zremrangebyscore("fastlane:driver_tokens:driver-1", 0, 15)

    assert removed == 1
    assert redis_client.zrange("fastlane:driver_tokens:driver-1", 0, -1) == ["token-b", "token-c"]
