...
from finance.business_date import derive_business_date_from_cutoff, resolve_store_business_date
...
class BusinessDateTests(SimpleTestCase):
    ...
    def test_derive_business_date_before_cutoff_rolls_back_one_day(self):
        ...
        business_date = derive_business_date_from_cutoff(reference, cutoff_hour=2, cutoff_minute=0)
    ...
    def test_resolve_store_business_date_prefers_explicit_value(self):
        ...
        business_date = resolve_store_business_date(store, explicit_business_date=timezone.datetime(2026, 6, 21).date())
        self.assertEqual(str(business_date), "2026-06-21")
