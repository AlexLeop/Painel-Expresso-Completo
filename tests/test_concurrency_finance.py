...
from finance.models import Wallet, WalletTransaction, Contract, OperatorInternalWallet
...
async def test_race_condition_settlement():
    ...
    client = await Client.objects.acreate(name="Test Client", document="12345678900", operator=operator)
    driver = await Driver.objects.acreate(operator=operator, name="Test Driver", active=True, phone="123")
    store = await Store.objects.acreate(operator=operator, client=client, name="Store", active=True)
    ...
    order = await Order.objects.acreate(
        operator=operator, store=store, driver=driver,
