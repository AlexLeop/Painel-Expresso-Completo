...
class TestTenantContext:
    ...
    def test_claims_contain_app_metadata_key(self):
        ...
        expected_claims = _json.dumps(
            {"app_metadata": {"operator_id": str(op_id)}, "role": "authenticated"}
        )
    ...
    def test_tenant_context_is_context_manager(self):
        ...
        assert inspect.isgeneratorfunction(
            tenant_context.__wrapped__
            if hasattr(tenant_context, "__wrapped__")
...
class TestDenyListSignal:
    ...
    def test_signal_is_post_save(self):
        ...
        sig_file = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "accounts", "signals.py"
        )
        with open(sig_file, "r", encoding="utf-8") as f:
            source = f.read()

        # Verificar os imports — a lógica real deve importar post_save
        assert "from django.db.models.signals import post_save" in source, (
            "DEVE importar post_save"
        )
        assert "from django.db.models.signals import pre_save" not in source, (
            "NÃO deve importar pre_save"
        )
...
class TestFernetKeyDerivation:
    ...
    def test_uses_hkdf_not_truncation(self):
        ...
        model_file = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "integration", "models.py"
        )
    ...
    def test_different_keys_produce_different_ciphers(self, mock_settings):
        ...
        assert encrypted_a != encrypted_b, (
            "Chaves diferentes devem produzir ciphertext diferentes"
        )
...
class TestTelemetryPipeline:
    ...
    def test_lrange_before_ltrim(self):
        ...
        task_file = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "logistics", "tasks.py"
        )
...
class TestWalletLocking:
    ...
    def test_get_or_create_before_select_for_update(self):
        ...
        assert "select_for_update().get(" in source, (
            "Deve usar select_for_update().get() separado"
        )
...
class TestFastLaneTokenIndex:
    ...
    def test_api_uses_zadd_not_sadd(self):
        ...
        api_file = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "logistics", "api_driver.py"
        )
        ...
        assert (
            '.sadd(f"fastlane:driver_tokens' not in source
            and ".sadd(f'fastlane:driver_tokens" not in source
        ), "NÃO deve usar SADD para tokens"
    ...
    def test_fast_lane_uses_zrem(self):
        ...
        fl_file = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "fast_lane", "main.py"
        )
    ...
    def test_time_import_at_top_level(self):
        ...
        for line in lines:
            ...
            if stripped == "import time" and line.startswith("    "):
                pytest.fail(
                    "'import time' está indentado (dentro de função). Deve estar no topo do módulo."
                )
...
class TestDockerBootstrap:
    ...
    def test_docker_compose_redis_unified_db(self):
        ...
        assert "redis:6379/2" not in content, (
            "Redis DB /2 não deve existir (unificado em /1)"
        )
...
class TestLogisticsSignals:
    ...
    def test_no_kill_switch_in_logistics_signals(self):
        ...
        assert "post_save" not in source, (
            "logistics/signals.py NÃO deve ter post_save handler"
        )
        assert "smembers" not in source, "NÃO deve ter smembers"
        assert "zrangebyscore" not in source, (
            "NÃO deve ter zrangebyscore (consolidado em accounts)"
        )
...
class TestSettings:
    ...
    def test_no_duplicate_celery_config(self):
        ...
        assert broker_count == 1, (
            f"CELERY_BROKER_URL aparece {broker_count} vezes (deve ser 1)"
        )

        backend_count = content.count("CELERY_RESULT_BACKEND")
        assert backend_count == 1, (
            f"CELERY_RESULT_BACKEND aparece {backend_count} vezes (deve ser 1)"
        )
