...
import { Truck, Clock, MapPin, CheckCircle2, Wifi, Circle, Navigation } from 'lucide-react';
...
export function FilaDinamica({ driverPositions, driverDirectory, busyDriverIds, storeLocation, onDriverClick }: FilaDinamicaProps) {
  const allDrivers = driverPositions.map(pos => {
    ...
    if (storeLocation && Number.isFinite(pos.latitude) && Number.isFinite(pos.longitude)) {
      distKm = haversineKm(storeLocation[0], storeLocation[1], pos.latitude, pos.longitude);
    }
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {onRide.length === 0 ? (
            ...
            onRide.map((driver) => (
              <div key={driver.id} onClick={() => onDriverClick?.(driver.id, true)} className="bg-white border border-amber-100 rounded-xl p-3 flex items-center gap-3 shadow-sm relative overflow-hidden cursor-pointer hover:border-amber-300 transition-all">
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-amber-500"></div>
                <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center shrink-0">
                  <Navigation className="w-4 h-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-bold text-zinc-900 truncate">{driver.name}</div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <div className="text-xs text-amber-600 font-medium flex items-center gap-1">
                      <Truck className="w-3 h-3" />
                      Em corrida
                    </div>
                    {driver.distKm !== null && Number.isFinite(driver.distKm) && (
        ...
        <div className="space-y-2">
          {active.length === 0 ? (
            ...
            active.map((driver, idx) => (
              <div key={driver.id} onClick={() => onDriverClick?.(driver.id, false)} className="bg-white border border-zinc-200 rounded-xl p-3 flex items-center gap-3 shadow-sm relative overflow-hidden group cursor-pointer hover:border-emerald-300 transition-all">
