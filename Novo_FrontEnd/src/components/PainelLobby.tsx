...
import { Package, Clock, MapPin, Truck, ChevronRight, CheckSquare, Square, Layers, User, Navigation } from 'lucide-react';
...
export function PainelLobby({ rides, storeLocation, companyData, onRidesGrouped }: PainelLobbyProps) {
  ...
  const handleGroupAndDispatch = async () => {
    ...
    try {
        ...
        paradas: selectedOrders.flatMap(order => {
          if (order.paradas && order.paradas.length > 0) {
            return order.paradas.map(p => ({
              ...
              observacao_parada: p.observacao || `Pedido #${order.codigoPedido}`,
            }));
          } else {
            return [{
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || 'Erro ao agrupar corrida na Machine API');
      }
      ...
      <div className="p-4 border-b border-zinc-200/80 bg-white sticky top-0 z-10 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-zinc-900 tracking-tight flex items-center gap-2">
            ...
            Lobby de Expedição
            <span className="text-zinc-400 font-medium ml-1 text-sm bg-zinc-100 px-2 py-0.5 rounded-full">{rides.length}</span>
          </h2>
          <p className="text-xs text-zinc-500 mt-1">Pedidos aguardando roteirização e despacho.</p>
        </div>
        ...
        {rides.length > 0 && (
          ...
          >
            {selectedIds.size === rides.length ? 'Desmarcar todos' : 'Selecionar todos'}
          </button>
        )}
      </div>
        ...
        <AnimatePresence>
          {rides.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm font-medium">Nenhum pedido pendente no lobby.</p>
            </motion.div>
          ) : (
            rides.map(order => {
              const isSelected = selectedIds.has(order.id);
              const minutesAgo = order.timestamp ? Math.max(0, Math.round((Date.now() - order.timestamp) / 60000)) : 0;
              const timeLabel = minutesAgo < 1 ? 'Agora' : minutesAgo < 60 ? `${minutesAgo} min` : `${Math.floor(minutesAgo / 60)}h${minutesAgo % 60}m`;
              const stopCount = Array.isArray(order.paradas) ? order.paradas.length : 0;
              return (
                <motion.div
                  key={order.id}
                  layout
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    minutesAgo > 15 ? "bg-rose-100 text-rose-700" :
                    minutesAgo > 5 ? "bg-amber-100 text-amber-700" :
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0 pr-16">
                      ...
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className="text-[10px] font-mono font-bold text-zinc-400 bg-zinc-100 px-1.5 py-0.5 rounded">#{order.codigoPedido}</span>
                        <span className="text-[9px] font-bold uppercase tracking-wider text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200/60">{order.status}</span>
                      </div>
                      
                      {/* Info row */}
                      <div className="flex items-center gap-2 text-[10px] text-zinc-500 mb-2">
                        <div className="flex items-center gap-1">
                          <Truck strokeWidth={1.5} className="w-3 h-3 shrink-0" />
                          <span className="truncate">{order.motoboy?.nome || 'Sem motorista'}</span>
                        </div>
                        <span className="text-zinc-300">•</span>
                        <div className="flex items-center gap-1">
                          <User strokeWidth={1.5} className="w-3 h-3 shrink-0" />
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-zinc-600 mb-1.5">
                        ...
                        <span className="truncate font-medium">{order.coleta?.endereco || 'Endereço não disponível'}</span>
                      </div>
                      
                      {/* Entrega address */}
                      <div className="flex items-center gap-1.5 text-xs text-zinc-500 mb-2">
                        <Navigation className="w-3 h-3 shrink-0 text-zinc-400" />
                        <span className="truncate">{order.entrega?.endereco || 'Destino'}</span>
                        {stopCount > 1 && (
                          <span className="text-[9px] font-bold text-zinc-400 bg-zinc-100 px-1 py-0.5 rounded">+{stopCount - 1}</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-3 text-[11px] font-semibold text-zinc-500">
                        ...
                        <span>•</span>
                        <span className="text-zinc-800 font-bold">{formatCurrency(order.valor)}</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
        ...
        {selectedIds.size > 0 && (
          ...
          >
            ...
            <p className="text-[10px] text-center text-zinc-400 mt-2">
              Será criada 1 corrida com {selectedIds.size} parada{selectedIds.size > 1 ? 's' : ''} para o 1º da Fila.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
