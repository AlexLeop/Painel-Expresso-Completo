...
import { RefreshCw, AlertCircle, CheckCircle2, Clock, XCircle, Play } from 'lucide-react';
...
const STATUS_CONFIG: Record<string, { label: string; colorClass: string; bgClass: string; icon: any }> = {
  pending: { label: 'Pendente', colorClass: 'text-amber-600', bgClass: 'bg-amber-50', icon: Clock },
  processing: { label: 'Processando', colorClass: 'text-blue-600', bgClass: 'bg-blue-50', icon: RefreshCw },
  completed: { label: 'Concluído', colorClass: 'text-emerald-600', bgClass: 'bg-emerald-50', icon: CheckCircle2 },
  failed: { label: 'Falhou', colorClass: 'text-orange-600', bgClass: 'bg-orange-50', icon: AlertCircle },
  dead: { label: 'DLQ', colorClass: 'text-rose-600', bgClass: 'bg-rose-50', icon: XCircle },
};
    ...
    <div className="bg-white rounded-xl shadow-sm ring-1 ring-zinc-950/5 overflow-hidden">
      <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/30">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-bold text-zinc-900 uppercase tracking-wider">Fila de Créditos</h2>
          {hasAlerts && (
          ...
          .map(([key, cfg]) => {
            ...
            return (
              <div key={key} className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-zinc-500 uppercase font-medium">{cfg.label}</span>
              </div>
            );
          })}
        {queueStats.totalAmount > 0 && (
          <div className="col-span-2 sm:ml-auto flex items-center gap-2 border-t sm:border-t-0 pt-2 sm:pt-0">
            <span className="text-[10px] text-zinc-400 uppercase font-bold tracking-wider">Total na fila:</span>
            <span className="text-sm font-bold text-zinc-900">{formatCurrency(queueStats.totalAmount)}</span>
          </div>
        )}
      </div>
        ...
        <div className="m-6 p-4 bg-rose-50 border border-rose-200 rounded-xl flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-rose-700 mt-0.5">
              Estes pagamentos falharam repetidamente e requerem intervenção manual.
            </p>
          </div>
          <button
              ...
              return (
                <tr key={item.id} className="hover:bg-zinc-50/50 transition-colors">
                  <td className="px-6 py-3.5">
                    <div className="font-bold text-zinc-900">{item.driver_name || item.machine_condutor_id}</div>
                    <div className="text-[10px] text-zinc-500 mt-0.5 max-w-[200px] truncate" title={item.description}>
                      {item.description || 'Sem descrição'}
                    </div>
                  </td>
                  <td className="px-4 py-3.5">
                    <span className="font-mono font-bold text-zinc-900">{formatCurrency(Number(item.net_amount))}</span>
                  </td>
                  <td className="px-4 py-3.5">
                    <div className={cn("inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[10px] font-bold uppercase tracking-wider", cfg.bgClass, cfg.colorClass, "border-current/10")}>
                  ...
                  <td className="px-4 py-3.5 text-zinc-500">
                    {item.status === 'failed' && item.next_retry_at ? nextRetryLabel(item.next_retry_at) : (item.status === 'dead' ? 'Manual' : '—')}
                  </td>
                  <td className="px-4 py-3.5">
                    {item.last_error && (
                      <div className="text-rose-500 max-w-[150px] truncate text-[10px]" title={item.last_error}>
                        {item.last_error}
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-3.5 text-right">
                    {(item.status === 'dead' || item.status === 'failed') && (
                      ...
                      >
                        <Play className={cn("h-4 w-4", retrying === item.id && "animate-pulse")} />
                      </button>
                    )}
                  </td>
                </tr>
