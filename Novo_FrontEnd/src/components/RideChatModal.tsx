...
import { X, Send, Image as ImageIcon, Search, Phone, ShieldCheck, Check, CheckCheck } from "lucide-react";
...
export function RideChatModal({ isOpen, onClose, corrida }: RideChatModalProps) {
        ...
        >
          ...
          <div className="p-4 border-b border-zinc-100 flex items-center justify-between bg-white z-10 shrink-0">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-full bg-zinc-100 flex items-center justify-center text-lg font-bold text-zinc-700">
                {corrida.motoboy?.nome ? corrida.motoboy.nome.charAt(0).toUpperCase() : "M"}
              </div>
              <div>
                <h3 className="text-sm font-bold text-zinc-900">{corrida.motoboy?.nome || "Motorista"}</h3>
          ...
          {/* Messages info banner */}
           <div className="bg-zinc-50 border-b border-zinc-100 px-4 py-2 flex items-center gap-2 shrink-0">
             ...
             <span className="text-[11px] text-zinc-500 font-medium">As mensagens são end-to-end e monitoradas para segurança.</span>
           </div>
                  ...
                  <div key={message.id} className={cn("flex w-full", isMe ? "justify-end" : "justify-start")}>
                    <div className={cn(
                      ...
                      <div className={cn(
                        ...
                        <span className="text-[10px] uppercase font-medium tracking-wider">{message.time}</span>
                        {isMe && (
                          message.status === 'read' ? <CheckCheck className="w-3 h-3 text-emerald-400" /> : <Check className="w-3 h-3" />
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div className="p-4 border-t border-zinc-100 bg-white shrink-0">
            <form onSubmit={handleSend} className="flex gap-2">
              <button type="button" className="p-2.5 text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100 rounded-xl transition-all shrink-0">
