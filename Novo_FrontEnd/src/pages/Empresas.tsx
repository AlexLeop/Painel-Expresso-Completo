...
import { Search, Plus, Filter, Store, MapPin, Phone, MoreVertical, Edit2, Trash2, X, Upload, Save, Building2, ClipboardList, Wallet, Image as ImageIcon, Power, PowerOff, Loader2, RefreshCw } from "lucide-react";
...
export function Empresas() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const fetchedEmpresas = rawList.map((c: any) => {
          const cfg = Array.isArray(c.company_configs) ? c.company_configs[0] : (c.company_configs || {});

          const rideFeePerDeliveryRaw =
            c.ride_fee_per_delivery ?? cfg.ride_fee_per_delivery ?? c.rideFeePerDelivery ?? c.ride_fee ?? null;
          const minimumRidesFeeFloorRaw =
            c.minimum_rides_fee_floor ?? cfg.minimum_rides_fee_floor ?? c.minimumRidesFeeFloor ?? null;
          const minimumFloorPercentRaw =
            c.minimum_floor_percent ?? cfg.minimum_floor_percent ?? c.minimumFloorPercent ?? null;
          const taxaSupervisaoRaw =
            c.taxa_supervisao ?? cfg.taxa_supervisao ?? c.taxaSupervisao ?? null;
          const debitoPendenteRaw =
            c.debito_pendente ?? cfg.debito_pendente ?? c.debitoPendente ?? null;

          const dailyRateWeekdayRaw =
            c.daily_rate_weekday ?? cfg.daily_rate_weekday ?? c.diaria_weekday ?? null;
          const dailyRateSaturdayRaw =
            c.daily_rate_saturday ?? cfg.daily_rate_saturday ?? c.diaria_saturday ?? null;
          const dailyRateSundayRaw =
            c.daily_rate_sunday ?? cfg.daily_rate_sunday ?? c.diaria_sunday ?? null;
          const dailyRateHolidayRaw =
            c.daily_rate_holiday ?? cfg.daily_rate_holiday ?? c.diaria_holiday ?? null;

          const extraKmModeRaw =
            c.extra_km_mode ?? cfg.extra_km_mode ?? c.extraKmMode ?? null;
          const extraKmMinDistanceRaw =
            c.extra_km_min_distance ?? cfg.extra_km_min_distance ?? c.extraKmMinDistance ?? null;
          const extraKmFixedAmountRaw =
            c.extra_km_fixed_amount ?? cfg.extra_km_fixed_amount ?? c.extraKmFixedAmount ?? null;
          const reportTypeRaw =
            c.report_type ?? cfg.report_type ?? c.reportType ?? null;
          const turnosRaw = c.turnos_config ?? cfg.turnos_config ?? (c.turnos || []);
          const faixasHorasRaw = c.faixas_horas_config ?? cfg.faixas_horas_config ?? (c.faixasHoras || []);
          ...
          return {
            ...
            // Regras Financeiras
            taxaCorridaPerEntrega: parseNum(rideFeePerDeliveryRaw, 1.60),
  ...
  const handleToggleStatus = async (id: any) => {
    ...
    try {
      setEmpresas(empresas.map(e => e.id === id ? { ...e, status: newStatus } : e));
      ...
      alert(`Falha ao atualizar status: ${err instanceof Error ? err.message : 'Erro desconhecido'}`);
    }
  };
  ...
  const confirmDelete = async () => {
    if (empresaToDelete !== null) {
      try {
        const response = await authFetch(`/api/db/companies/${empresaToDelete}`, {
          method: 'DELETE'
        });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 py-2">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 tracking-tight">Empresas</h1>
          <p className="text-[13px] font-medium text-zinc-500 mt-1">Gestão de regras financeiras para lojas sincronizadas da Machine API.</p>
        </div>
      </div>
      ...
      <div className="glass-panel overflow-hidden flex flex-col">
        <div className="p-4 border-b border-zinc-100 flex flex-col sm:flex-row gap-3 items-center justify-between bg-zinc-50/30">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            >
              <RefreshCw strokeWidth={1.5} className={cn("h-4 w-4", loading && "animate-spin")} /> 
              ...
              {filteredEmpresas.length === 0 && !error && !loading && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-zinc-500">
                    {searchTerm ? 'Nenhuma empresa encontrada para a busca.' : 'Nenhuma empresa encontrada.'}
                  </td>
                </tr>
              )}
              {filteredEmpresas.map((empresa) => (
                <motion.tr variants={itemVariants} key={empresa.id} className="hover:bg-zinc-50/80 transition-colors group">
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-zinc-900 block">{empresa.nome}</span>
                    </div>
                  </td>
                  <td className="px-4 py-2">
                    <div className="flex flex-col">
                      <div className="flex items-center gap-1.5 text-xs text-zinc-600">
                        <MapPin strokeWidth={1.5} className="h-3 w-3 text-zinc-400" />
                        <span className="truncate max-w-[200px]">{empresa.endereco}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-[11px] text-zinc-500 mt-0.5">
                        <Phone strokeWidth={1.5} className="h-3 w-3 text-zinc-400" />
                        {empresa.telefone}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-2">
                    <span className={cn(
                      ...
                      empresa.status === "Ativo" ? "bg-emerald-50 text-emerald-700 border border-emerald-200/60" : "bg-red-50 text-red-700 border border-red-200/60"
                    )}>
                      {empresa.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2 text-right">
                    <div className="flex flex-col items-end">
                      <span className="font-mono text-xs text-zinc-900 font-bold">
                        {empresa.extraKmMode === 'disabled' ? '—' : (empresa.extraKmMode === 'fixed' ? formatCurrency(empresa.extraKmFixedAmount) : 'Taxa')}
                      </span>
                  ...
                  <td className="px-4 py-2 text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      >
                        {empresa.status === "Ativo" ? <PowerOff strokeWidth={1.5} className="h-3.5 w-3.5" /> : <Power strokeWidth={1.5} className="h-3.5 w-3.5" />}
                      </button>
                      <button 
                      ...
                      <button 
                        onClick={() => empresa.id && handleDeleteEmpresa(empresa.id)}
                        className="p-1 px-1.5 text-zinc-400 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-colors" title="Excluir">
        ...
        <div className="px-4 py-3 border-t border-zinc-200 bg-zinc-50 flex items-center justify-center">
          <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider">Mostrando {filteredEmpresas.length} de {empresas.length} empresas</span>
        </div>
      </div>
