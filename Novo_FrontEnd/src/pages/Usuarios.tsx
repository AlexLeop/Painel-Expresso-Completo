...
import { Search, Plus, Filter, UserCog, User, Phone, MoreVertical, Edit2, Trash2, Power, PowerOff, Loader2 } from "lucide-react";
    ...
    try {
      ...
      if (res.ok) {
        ...
        const ROLE_LABEL: Record<string, string> = {
          ...
          coordinator: 'Coordenador', operator: 'Operador', viewer: 'Visualizador',
        };
        const fetchedUsers = rawList.map((u: any) => ({
          ...
          empresas: Array.isArray(u.companies) ? u.companies.map((c: any) => c.id || c) : (u.empresas || []),
        }));
  ...
  const handleSaveUser = async (user: UserType) => {
    try {
      const ROLE_TO_API: Record<string, string> = {
        'Administrador': 'admin', 'Gestor': 'manager', 'Supervisor': 'supervisor',
        'Coordenador': 'coordinator', 'Operador': 'operator', 'Visualizador': 'viewer',
      };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl shadow-sm ring-1 ring-zinc-950/5">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 tracking-tight">Usuários</h1>
          <p className="text-sm text-zinc-500 mt-1">Gerencie os acessos ao painel Administrativo do sistema.</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-[0_1px_3px_0_rgba(0,0,0,0.05),_0_0_0_1px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-zinc-100">
              {error && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-red-500 bg-red-50/50">
              ...
              {filteredUsuarios.map((usuario) => (
                <motion.tr variants={itemVariants} key={usuario.id} className="hover:bg-zinc-50/80 transition-colors group">
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="font-semibold text-zinc-900 block text-sm">{usuario.nome}</span>
                  ...
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-1.5 text-xs text-zinc-700 font-medium">
                      <UserCog strokeWidth={1.5} className="h-3.5 w-3.5 text-zinc-400" />
                      {usuario.cargo}
                    </div>
                  </td>
                  <td className="px-4 py-2">
                    <span className={cn(
                      ...
                      usuario.status === "Ativo" ? "bg-emerald-50 text-emerald-700 border border-emerald-200/60" : "bg-zinc-100 text-zinc-600 border border-zinc-200/60"
                    )}>
                      {usuario.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2 text-right">
                    <div className="flex justify-end gap-1">
                      <button 
                        onClick={() => usuario.id && handleToggleStatus(usuario.id)}
                        className={cn(
                      ...
                      >
                        {usuario.status === "Ativo" ? <PowerOff strokeWidth={1.5} className="h-3.5 w-3.5" /> : <Power strokeWidth={1.5} className="h-3.5 w-3.5" />}
                      </button>
                      <button 
                      ...
                      <button 
                        onClick={() => usuario.id && handleDeleteUser(usuario.id)}
        ...
        <div className="px-4 py-3 border-t border-zinc-200 bg-zinc-50 flex items-center justify-center">
          <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider">Mostrando {filteredUsuarios.length} de {usuarios.length} usuários</span>
        </div>
      </div>
