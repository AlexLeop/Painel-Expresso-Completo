  ...
  return (
    <button
      ...
      style={{
        ...
        background: active ? 'linear-gradient(135deg, #E55C00, #CC5200)' : '#FFFFFF',
...
export function Login() {
  ...
  const handleRegister = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        const msg = data?.details?.errors?.[0]?.message || data?.error || 'Erro ao cadastrar. Tente novamente.';
        setRegisterError(String(msg));
      }
    } catch {
      setRegisterError('Erro de conexão. Verifique sua internet e tente novamente.');
    } finally {
  ...
  const handleRecover = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        setRecoverError('Não foi possível iniciar a recuperação. Verifique o e-mail e tente novamente.');
        return;
      }
      setRecoverMessage('Se este e-mail estiver cadastrado, você receberá instruções para redefinir a senha.');
    } finally {
  ...
  const handleSetPassword = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResetSuccess('Senha atualizada com sucesso. Faça login para continuar.');
      try {
  ...
  const footer = (
    <div style={{ marginTop: '12px', textAlign: 'center', fontSize: '0.7rem', color: '#999999' }}>
      ...
      <div className="login-container">
        ...
        <div className="login-card-wrapper">
          <div className="login-card" style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#333333', letterSpacing: '-0.03em' }}>
              Cadastro enviado!
            </h2>
            <p style={{ fontSize: '0.9rem', color: '#666666', marginTop: '8px', lineHeight: 1.5 }}>
              Sua empresa foi registrada e está aguardando aprovação.
            </p>
            {contractNumber && (
              <div style={{ marginTop: '14px', fontSize: '0.85rem', color: '#333333' }}>
      ...
      <div className="login-card-wrapper">
        <div className="login-card">
          <div style={{ display: 'flex', gap: '10px', marginBottom: '18px' }}>
            <TabButton active={mode === 'login'} label="Entrar" onClick={() => setMode('login')} />
            <TabButton active={mode === 'register'} label="Cadastro" onClick={() => setMode('register')} />
          </div>

          {mode === 'recover' && (
            <>
              <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                <h2 style={{ fontSize: '1.35rem', fontWeight: 800, color: '#333333', letterSpacing: '-0.03em' }}>
                  Recuperar Senha
                </h2>
                <p style={{ fontSize: '0.82rem', color: '#999999', marginTop: '6px' }}>
                  Enviaremos um link para redefinir sua senha.
                </p>
              </div>

              {recoverError && (
                <div style={{ padding: '10px 14px', borderRadius: '10px', background: '#FEF2F2', border: '1px solid #FECACA', color: '#DC2626', fontSize: '0.8rem', fontWeight: 500, marginBottom: '14px', textAlign: 'center' }}>
                  {recoverError}
                </div>
              )}
              {recoverMessage && (
                <div style={{ padding: '10px 14px', borderRadius: '10px', background: '#ECFDF5', border: '1px solid #A7F3D0', color: '#047857', fontSize: '0.8rem', fontWeight: 500, marginBottom: '14px', textAlign: 'center' }}>
                  {recoverMessage}
                </div>
              )}
              ...
              <form onSubmit={handleRecover}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    borderRadius: '10px',
                    background: recoverLoading ? '#FFB380' : 'linear-gradient(135deg, #E55C00, #CC5200)',
                    color: 'white',
                    fontSize: '0.9rem',
                    fontWeight: 700,
                ...
                <div style={{ marginTop: '16px', textAlign: 'center' }}>
                  <button type="button" onClick={() => setMode('login')} style={{ background: 'none', border: 'none', color: '#E55C00', fontWeight: 800, cursor: 'pointer', fontSize: '0.82rem' }}>
                    Voltar ao Login
                  </button>
                </div>
              </form>
              ...
              <form onSubmit={handleSetPassword}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    borderRadius: '10px',
                    background: resetLoading ? '#FFB380' : 'linear-gradient(135deg, #E55C00, #CC5200)',
                    color: 'white',
                    fontSize: '0.9rem',
                    fontWeight: 700,
          ...
          {mode === 'login' && (
            <>
              <div style={{ textAlign: 'center', marginBottom: '22px' }}>
                <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#333333', letterSpacing: '-0.03em' }}>
                  Entrar no Painel
                </h2>
                <p style={{ fontSize: '0.82rem', color: '#999999', marginTop: '6px' }}>
              ...
              {loginError && (
                <div style={{ padding: '10px 14px', borderRadius: '10px', background: '#FEF2F2', border: '1px solid #FECACA', color: '#DC2626', fontSize: '0.8rem', fontWeight: 500, marginBottom: '18px', textAlign: 'center' }}>
                  {loginError}
                </div>
              )}
              ...
              <form onSubmit={handleLogin}>
                ...
                <div style={{ marginBottom: '18px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: '0.78rem', fontWeight: 800, color: '#E55C00' }}
                    >
                      Esqueci minha senha
                    </button>
                  </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    borderRadius: '10px',
                    background: loginLoading ? '#FFB380' : 'linear-gradient(135deg, #E55C00, #CC5200)',
                    color: 'white',
                    fontSize: '0.9rem',
                    fontWeight: 700,
          ...
          {mode === 'register' && (
            <>
              ...
              <form onSubmit={handleRegister}>
                <input type="text" value={registerForm.website} onChange={(e) => setRegisterForm(p => ({ ...p, website: e.target.value }))} style={{ display: 'none' }} tabIndex={-1} autoComplete="off" />

                <div style={{ marginBottom: '14px' }}>
                  <label style={labelStyle}>Nome Fantasia</label>
                  <input
                    value={registerForm.nome_fantasia}
                    onChange={(e) => setRegisterForm(p => ({ ...p, nome_fantasia: e.target.value }))}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '14px' }}>
                  <div>
                    ...
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (registerForm.documento && !validateCNPJ(registerForm.documento)) {
                          setCnpjError('CNPJ inválido. Verifique os dígitos.');
                        }
                      }}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setRegisterForm(p => ({ ...p, telefone: formatPhone(e.target.value) }))}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '12px', marginBottom: '14px' }}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setRegisterForm(p => ({ ...p, endereco: e.target.value }))}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '14px' }}>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setRegisterForm(p => ({ ...p, complemento: e.target.value }))}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 70px', gap: '12px', marginBottom: '14px' }}>
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setRegisterForm(p => ({ ...p, uf: e.target.value.toUpperCase().slice(0, 2) }))}
                      placeholder="UF"
                      style={inputStyle}
                      onFocus={(e) => (e.currentTarget.style.borderColor = '#E55C00')}
                      onBlur={(e) => (e.currentTarget.style.borderColor = '#E5E5E5')}
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    borderRadius: '10px',
                    background: registerSubmitting ? '#FFB380' : 'linear-gradient(135deg, #E55C00, #CC5200)',
                    color: 'white',
                    fontSize: '0.9rem',
                    fontWeight: 700,
