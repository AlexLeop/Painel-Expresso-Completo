...
export function Financeiro() {
  ...
  const currentCompany = companies?.find((c: any) => Number(c.id) === companyId);
  const companyName = currentCompany?.nome || "Empresa";
  const isAdmin = user?.role === 'admin' || user?.role === 'master' || user?.role === 'administrador';

  const [activeTab, setActiveTab] = useState<'overview' | 'queue' | 'log' | 'balances' | 'acerto'>('acerto');
  ...
  const loadDriverBalances = useCallback(async () => {
    ...
    try {
      ...
      const resDrivers = await authFetch(`/api/db/company-drivers?company_id=${companyId}&active_only=0`);
      ...
      for (const driver of drivers) {
        try {
          const res = await authFetch(`/api/machine/credits/driver/balance?condutor_id=${driver.id}`);
          if (res.ok) {
            const data = await res.json();
            setDriverBalances(prev => prev.map(db => 
              db.id === driver.id ? { ...db, balance: data.saldo || 0, loading: false } : db
            ));
          } else {
            setDriverBalances(prev => prev.map(db => 
              db.id === driver.id ? { ...db, loading: false, error: 'Erro API' } : db
            ));
          }
        } catch {
          setDriverBalances(prev => prev.map(db => 
            db.id === driver.id ? { ...db, loading: false, error: 'Erro rede' } : db
          ));
        }
      }
    } catch (err) {
    ...
    try {
      ...
      if (res.ok && data.results) {
        const lines = (data.results as any[]).map(r => {
          if (r.status === 'credited') return `✓ ${r.driver}: ${formatCurrency(r.amount)}`;
          if (r.status === 'skipped') return `○ ${r.driver}: ${r.reason || 'sem pendência'}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-5 rounded-xl shadow-sm ring-1 ring-zinc-950/5">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 tracking-tight">Financeiro</h1>
          <p className="text-sm text-zinc-500 mt-1">{companyName} • Gestão de créditos automáticos</p>
        </div>
        <div className="flex items-center gap-3 mt-4 sm:mt-0">
          ...
          >
            {processing ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Clock className="h-4 w-4" />}
            {processing ? 'Processando...' : `Creditar Ontem (${yesterday.split('-').reverse().slice(0, 2).join('/')})`}
          </button>
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Diárias na Semana" value={stats.total} icon={TrendingUp} color="blue" />
        <StatCard label="Pendentes" value={stats.pending} icon={Clock} color="orange" />
        <StatCard label="Creditadas" value={stats.credited} icon={CheckCircle2} color="green" />
        <StatCard label="Falhas" value={stats.failed} icon={AlertCircle} color="red" />
      </div>
      ...
      {(config || processingResults.length > 0) && (
        <div className="bg-white rounded-xl shadow-sm ring-1 ring-zinc-950/5 p-5">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h3 className="text-sm font-bold text-zinc-900 uppercase tracking-wider">Configuração de Crédito</h3>
          ...
          {processingResults.length > 0 && (
            <div className="mt-4 p-3 bg-zinc-50 rounded-lg border border-zinc-100 max-h-40 overflow-y-auto">
              {processingResults.map((res, i) => (
                <div key={i} className="text-[11px] font-mono text-zinc-600 py-0.5 border-b border-zinc-200/50 last:border-0">
                  {res}
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {config && (
              <div className="md:col-span-3 bg-indigo-50 border border-indigo-100 p-5 rounded-xl">
                <h4 className="text-sm font-bold text-indigo-900 uppercase tracking-wider mb-2">Regra de Extras por Km</h4>
                <p className="text-xs text-indigo-700 leading-relaxed">
                  {config.extraKm.mode === 'disabled' && 'Esta loja não possui pagamento de extra por KM excedente configurado.'}
                  {config.extraKm.mode === 'fixed' && `Valor fixo de ${formatCurrency(config.extraKm.fixedAmount)} para corridas acima de ${config.extraKm.minKm}km.`}
                  {config.extraKm.mode === 'delivery_fee' && `Taxa adicional de entrega (${formatCurrency(config.taxaCorridaPerEntrega)}) para corridas acima de ${config.extraKm.minKm}km.`}
                </p>
              </div>
            )}
          </div>
                  ...
                  {creditLog.length === 0 ? (
                    ...
                    creditLog.map((entry) => (
                      <tr key={entry.id} className="hover:bg-zinc-50/50 transition-colors">
                        ...
                        <td className="px-4 py-4 text-zinc-700 font-medium">{entry.driverName}</td>
                        <td className="px-4 py-4 text-right font-mono">{formatCurrency(entry.breakdown.diaria)}</td>
                        <td className="px-4 py-4 text-right font-mono text-emerald-600">
                          {entry.breakdown.extras > 0 ? `+${formatCurrency(entry.breakdown.extras)}` : '—'}
                        </td>
                        <td className="px-4 py-4 text-right font-mono text-rose-500">
                          {entry.breakdown.adiantamentos > 0 ? `-${formatCurrency(entry.breakdown.adiantamentos)}` : '—'}
                        </td>
                        <td className={cn(
                          "px-4 py-4 text-right font-mono font-bold",
                          entry.amount >= 0 ? "text-zinc-900" : "text-rose-600"
                        )}>
        ...
        {activeTab === 'balances' && (
          <div className="bg-white rounded-xl shadow-sm ring-1 ring-zinc-950/5 overflow-hidden">
            <div className="p-5 border-b border-zinc-200 flex justify-between items-center bg-zinc-50/50">
              <div>
                <h3 className="text-sm font-bold text-zinc-900 uppercase tracking-wider">Saldos na Plataforma</h3>
                <p className="text-xs text-zinc-500 mt-1">Saldos atualizados em tempo real na carteira da Machine.</p>
              </div>
              <button 
              ...
              >
                <RefreshCw className={cn("h-3.5 w-3.5", loadingBalances && "animate-spin")} />
                Atualizar
              </button>
            </div>
              ...
              <table className="w-full text-left whitespace-nowrap text-sm">
                <thead className="bg-zinc-50/80 border-b border-zinc-200 text-zinc-500 text-[10px] font-bold uppercase tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-right">Saldo Atual (Machine)</th>
                  ...
                  {driverBalances.length === 0 && !loadingBalances ? (
                    ...
                    driverBalances.map((driver) => (
                      <tr key={driver.id} className="hover:bg-zinc-50/50 transition-colors">
                        <td className="px-6 py-4 font-bold text-zinc-900">{driver.name}</td>
                        <td className="px-6 py-4 text-right">
                          {driver.loading ? (
                            ...
                            <span className="text-rose-500 font-medium text-xs">{driver.error}</span>
                          ) : (
                            <span className={cn(
                              "font-mono font-bold text-base",
                              (driver.balance || 0) >= 0 ? "text-zinc-900" : "text-rose-600"
                            )}>
...
function InfoBox({ step, title, description, color }: { step: string; title: string; description: string; color: string }) {
  ...
  return (
    <div className={cn("bg-white p-6 rounded-xl shadow-sm ring-1 ring-zinc-950/5 border-t-4", borderColors[color])}>
      <div className="flex items-center gap-3 mb-3">
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-zinc-900 text-white text-[10px] font-bold">
          {step}
        </span>
        <h4 className="font-bold text-zinc-900 uppercase tracking-wider text-sm">{title}</h4>
      </div>
