...
import { Search, Filter, MapPin, Navigation, Clock, Truck, Phone, MessageSquare, ShieldCheck, Mail, Map as MapIcon, ChevronRight, Store, FileDown, Plus, User, Loader2, Ban, ExternalLink, Layers, FileText } from "lucide-react";
...
function MapController({ coords, rideId, focusKey }: { coords: [number, number] | null, rideId: string | number | undefined, focusKey?: number }) {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    if (rideId !== lastRideId.current || (focusKey !== undefined && focusKey !== lastFocusKey.current)) {
...
function normalizeLatLng(latRaw: unknown, lngRaw: unknown): { lat: number; lng: number } | null {
  ...
  const inRange = (a: number, b: number) => Math.abs(a) <= 90 && Math.abs(b) <= 180;
...
export function Corridas() {
  ...
  const [leftPanelMode, setLeftPanelMode] = useState<'lobby' | 'fila' | 'machine'>('lobby');
  const [selectedHistoryMap, setSelectedHistoryMap] = useState<any | null>(null);
  const [activeSubTab, setActiveSubTab] = useState<'aguardando' | 'despachado'>('aguardando');
  ...
  const [machineCompanyData, setMachineCompanyData] = useState<any | null>(null);
  ...
  const [driverDirectory, setDriverDirectory] = useState<Record<string, { name: string; phone: string; status: string }>>({});
  ...
  const currentCompany = useMemo(() => {
    const found = user?.companies?.find((c: any) => String(c.id) === String(companyId));
  ...
  }, [mapDebug, onlineCount, posByDriverId, activeRides.length, selectedDriverId]);
  ...
  const handleOpenTracking = async (idMch: string | number) => {
    ...
    try {
      ...
      if (res.ok && (data.response?.[0]?.link_rastreio || data.links?.[0]?.link_rastreio)) {
        const url = data.response?.[0]?.link_rastreio || data.links?.[0]?.link_rastreio;
        window.open(url, '_blank', 'noopener,noreferrer');
        showToast('Abrindo link de rastreio ao vivo em nova aba');
      } else {
        const errorMsg = data?.details?.errors?.[0]?.message || data?.error || 'Link não disponível para esta corrida';
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const STATUS_LABEL: Record<string, string> = {
            D: 'Aguardando', G: 'Aguardando', P: 'Aguardando',
            A: 'Aceita', E: 'Em andamento', S: 'A caminho',
            F: 'Concluída', C: 'Cancelada', N: 'Não atendida',
          };

          const mapRide = (r: any) => {
            const rideEmpresaId = String(r.empresa_id);
            const rideEmpresaNome = currentCompanies[rideEmpresaId] || r.nome_empresa || rideEmpresaId || 'Empresa';
            
            const p0 = r.paradas?.[0] || {};
            let rawCode = r.identificador_solicitacao || r.identificador || r.codigo_pedido || r.numero_pedido || r.pedido_id || p0.codigo_pedido || p0.id_pedido || p0.numero_pedido || '';
            if (String(rawCode).trim() === '0001' || String(rawCode).trim() === '1' || !rawCode) {
              const obs = [r.observacao, r.observacoes, r.dados_extras, p0.observacao_parada, p0.observacao].filter(Boolean).join(' ');
              const m = obs.match(/(?:pedido|ped|order|id|código)[\s:#-]+(\d{2,15})/i);
              if (m && m[1]) {
                rawCode = m[1];
              }
            }
            ...
            let clienteNome = r.cliente_nome || p0.nome_contato || p0.nome_cliente || '';
            
            if (!clienteNome || String(clienteNome).trim() === '') {
              const obsText = [p0.observacao_parada, p0.observacao, r.observacao, r.observacoes, r.dados_extras, p0.complemento].filter(Boolean).join(' ');
              const match = obsText.match(/(?:cliente|contato|para|nome|sr|sra|recebedor|a\/c)[:\s-]+([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ\s]{2,35})/i);
              if (match && match[1]) {
            ...
            const telefoneCliente = r.cliente_telefone || p0.telefone_contato || p0.telefone_cliente || p0.telefone_cliente_parada || p0.telefone || r.telefone_passageiro || r.telefone_destino || '';

            const stopCount = Array.isArray(r.paradas) ? r.paradas.length : 0;
            const tipoPedido =
              stopCount > 1 ? 'Multi-entrega' :
              r.com_retorno ? 'Com retorno' :
              'Simples';

            return {
              id: r.id,
              codigoPedido,
              status: STATUS_LABEL[r.status_solicitacao] || r.status_solicitacao || 'Desconhecido',
              rawStatus: r.status_solicitacao,
              tipo: tipoPedido,
              horario: r.data_hora_solicitacao ? r.data_hora_solicitacao.split(' ')[1] : '',
              data: r.data_hora_solicitacao ? r.data_hora_solicitacao.split(' ')[0].split('-').reverse().join('/') : '',
              cliente: clienteNome,
              telefoneCliente,
              empresa: rideEmpresaNome,
              distancia: r.distancia_percorrida_km ? `${parseFloat(r.distancia_percorrida_km).toFixed(1)} km` : '—',
              valor: parseFloat(r.valor_corrida || '0'),
              telefoneMotorista: r.telefone_condutor || r.telefone_taxista || '',
              motoboy: {
              ...
              coleta: {
                ...
                hora: r.data_hora_solicitacao ? r.data_hora_solicitacao.split(' ')[1].substring(0, 5) : '--:--',
              },
              entrega: {
                endereco: r.paradas?.[0]?.endereco || '',
                hora: r.data_hora_finalizacao ? r.data_hora_finalizacao.split(' ')[1].substring(0, 5) : '--:--',
              },
              origin: (r.coleta?.latitude ?? r.coleta?.lat) && (r.coleta?.longitude ?? r.coleta?.lng)
                ? { lat: parseFloat(String(r.coleta.latitude ?? r.coleta.lat)), lng: parseFloat(String(r.coleta.longitude ?? r.coleta.lng)) }
                : null,
              destination: (r.paradas?.[0]?.latitude ?? r.paradas?.[0]?.lat) && (r.paradas?.[0]?.longitude ?? r.paradas?.[0]?.lng)
                ? { lat: parseFloat(String(r.paradas[0].latitude ?? r.paradas[0].lat)), lng: parseFloat(String(r.paradas[0].longitude ?? r.paradas[0].lng)) }
                : null,
              lastLoc: r.coleta?.cidade || 'Desconhecido',
              lastStop: r.paradas?.[0]?.endereco || 'Nenhuma',
              speed: '—',
              timestamp: r.data_hora_solicitacao ? new Date(r.data_hora_solicitacao.replace(' ', 'T')).getTime() : 0,
              paradas: r.paradas || [],
          ...
          const sortedRides = [...rawRides].sort((a, b) => getTimestamp(b) - getTimestamp(a));

          const inProgress = sortedRides.filter(r => STATUS_ATIVO.has(r.status_solicitacao)).map(mapRide);
          const completed = sortedRides.filter(r => ['F', 'C', 'N'].includes(r.status_solicitacao)).map(mapRide);
        ...
        try {
          ...
          if (compRes.ok) {
            const compData = await compRes.json();
            const myComp = compData.companies?.find((c: any) => String(c.id) === companyIdStr);
            if (myComp) {
              setMachineCompanyData(myComp);
              if (myComp.lat && myComp.lng) {
                setStoreLocation([parseFloat(myComp.lat), parseFloat(myComp.lng)]);
              } else if (myComp.latitude && myComp.longitude) {
                setStoreLocation([parseFloat(myComp.latitude), parseFloat(myComp.longitude)]);
              }
            }
          }
        } catch (err) {
        ...
        try {
          const companyIdStr = String(companyId);
          if (companyIdStr) {
            const driversRes = await authFetch(`/api/db/company-drivers?company_id=${companyIdStr}&active_only=0`);
            if (driversRes.ok) {
              const rows = await driversRes.json();
              const next: Record<string, { name: string; phone: string; status: string }> = {};
              if (Array.isArray(rows)) {
                for (const r of rows) {
                  const row = r as { driverId?: unknown; driverName?: unknown; driverPhone?: unknown; driverStatus?: unknown };
        ...
        try {
          ...
          if (machineDriversRes.ok) {
            ...
            const drivers = Array.isArray(machineDriversData?.drivers) ? machineDriversData.drivers : [];
            setDriverDirectory(prev => {
              const merged = { ...prev };
              for (const d of drivers) {
                const row = d as { id?: unknown; nome?: unknown; telefone?: unknown; status?: unknown };
          ...
          if (mapDebug) console.debug('[Mapa] Falha ao buscar motoboys (Machine):', err);
        }

        try {
          const posRes = await authFetch('/api/db/positions?max_age_minutes=720&limit=1000');
          if (posRes.ok) {
            const posData = await posRes.json();
            const positionsRaw = Array.isArray(posData.positions) ? posData.positions : [];
            const normalized: DriverPosition[] = [];
            let dropped = 0;
            for (const p of positionsRaw) {
              const row = p as { machine_condutor_id?: unknown; latitude?: unknown; longitude?: unknown; speed?: unknown; heading?: unknown; machine_ride_id?: unknown; received_at?: unknown };
              ...
              normalized.push({
                ...
                machine_ride_id: row.machine_ride_id ? String(row.machine_ride_id) : null,
                received_at: receivedAt,
              });
            }
            setDriverPositions(normalized);
            knownDriverIdsRef.current = new Set(normalized.map(p => String(p.machine_condutor_id)));
            if (mapDebug) {
    ...
    const flushBuffer = () => {
      ...
      setDriverPositions(prev => {
        const map = new Map(prev.map(p => [String(p.machine_condutor_id), p]));
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const newRecord = payload.new as { machine_condutor_id?: unknown; latitude?: unknown; longitude?: unknown; speed?: unknown; heading?: unknown; machine_ride_id?: unknown; received_at?: unknown };
          const id = String(newRecord?.machine_condutor_id || '');
          if (!id) return;

          const allowed = companyDriverIdsRef.current.has(id) || knownDriverIdsRef.current.has(id) || activeDriverIdsRef.current.has(id);
          if (!allowed) return;

          const coords = normalizeLatLng(newRecord?.latitude, newRecord?.longitude);
          if (!coords) return;
          ...
          positionBuffer.set(id, {
            ...
            machine_ride_id: newRecord.machine_ride_id ? String(newRecord.machine_ride_id) : null,
            received_at: String(newRecord.received_at || new Date().toISOString()),
          });
  ...
  const handleExportCSV = () => {
    ...
    const rows = historyRides.map(r =>
      [r.id, r.status, r.tipo, r.cliente, r.empresa, r.horario, r.valor].map(v => escapeCSV(v)).join(';')
    ).join('\n');
    const bom = '\uFEFF';
  ...
  const filteredActiveRides = useMemo(() => activeRides.filter(r => {
    const aTerm = activeSearch.toLowerCase().trim();
    const gTerm = globalSearch.toLowerCase().trim();

    const checkTerm = (term: string) => !term ? true : (
  ...
  const aguardandoRides = useMemo(() => {
    ...
    return filteredActiveRides.filter(r => {
      const raw = String(r.rawStatus || '').trim().toUpperCase();
  ...
  const handleDriverClick = useCallback((driverId: string, isOnRide: boolean) => {
    // Switch to Radar tab
    ...
    if (isOnRide) {
      ...
      const ride = activeRides.find(r => {
        const raw = String(r.rawStatus || '').trim().toUpperCase();
        return (raw === 'A' || raw === 'E') && String(r.motoboy?.id_num || '').trim() === driverId;
      });
      if (ride) {
        setSelectedCorrida(ride);
  ...
  useEffect(() => {
    const currentList = activeSubTab === 'aguardando' ? aguardandoRides : despachadoRides;
    if (selectedCorrida) {
  ...
  const filteredHistoryRides = useMemo(() => historyRides.filter(r => {
    ...
    const st = String(r.status || '').trim().toLowerCase();
    const matchesFilter = historyFilter === "Todas" ? true :
                          historyFilter === "Concluídas" ? (st === "concluída" || st === "concluido" || st === "concluído") :
                          historyFilter === "Canceladas" ? (st === "cancelada" || st === "não atendida" || st === "cancelado") :
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row h-full min-h-0 bg-white overflow-hidden" style={{ minHeight: '100%' }}>
      {/* Left Sidebar */}
      <div className="w-full lg:w-[360px] bg-white border-r border-zinc-200 flex flex-col shrink-0 z-10 min-h-0">
        ...
        <div className="px-4 pt-4 bg-zinc-50/50">
          <div className="flex bg-zinc-200/60 p-1 rounded-xl">
             <button 
                ...
                className={cn("flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5", leftPanelMode === 'lobby' ? "bg-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] text-zinc-900" : "text-zinc-500 hover:text-zinc-700")}
             >
             ...
             <button 
                ...
                className={cn("flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5", leftPanelMode === 'fila' ? "bg-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] text-zinc-900" : "text-zinc-500 hover:text-zinc-700")}
             >
             ...
             <button 
                ...
                className={cn("flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5", leftPanelMode === 'machine' ? "bg-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] text-zinc-900" : "text-zinc-500 hover:text-zinc-700")}
             >
        ...
        {leftPanelMode === 'lobby' ? (
          ...
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-bold text-zinc-900 tracking-tight">Corridas ativas <span className="text-zinc-400 font-medium ml-1">{filteredActiveRides.length}</span></h2>
            <button onClick={() => { setRedistributeRideData(null); setIsCreateModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 bg-zinc-900 text-white rounded-lg hover:bg-zinc-800 text-sm font-bold shadow-md transition-all focus:ring-2 focus:ring-zinc-900/20 active:scale-95">
              <Plus strokeWidth={2.5} className="h-4 w-4" /> Nova Entrega
            </button>
          </div>
          <div className="flex gap-2">
            <div className="relative flex-1 group">
              <Search strokeWidth={1.5} className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400 group-focus-within:text-zinc-600 transition-colors" />
              <input 
                type="text" 
                placeholder="Buscar..." 
                value={activeSearch}
            ...
            <div className="relative">
              <button 
                onClick={() => setActiveFilter(prev => prev === "Todos" ? "Aguardando" : prev === "Aguardando" ? "Em andamento" : "Todos")}
                title={`Filtro atual: ${activeFilter}`}
                className={cn("px-3 h-full flex items-center justify-center border text-xs font-semibold rounded-lg transition-all shadow-sm", activeFilter !== "Todos" ? "bg-zinc-900 text-white border-zinc-900" : "bg-white border-zinc-200 text-zinc-700 hover:bg-zinc-50")}
              >
        ...
        <div className="flex border-b border-zinc-100 bg-zinc-50/30 px-3 shrink-0">
          ...
          >
            ...
            <span className={cn(
              ...
              activeSubTab === "aguardando" ? "bg-zinc-900 text-white" : "bg-zinc-200 text-zinc-600"
            )}>
          ...
          >
            ...
            <span className={cn(
              "px-1.5 py-0.5 rounded-full text-[10px] font-bold",
              activeSubTab === "despachado" ? "bg-zinc-900 text-white" : "bg-zinc-200 text-zinc-600"
            )}>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-zinc-50/30">
          {error ? (
            ...
            const minutesAgo = corrida.timestamp ? Math.max(0, Math.round((Date.now() - corrida.timestamp) / 60000)) : 0;
            const timeLabel = minutesAgo < 1 ? 'Agora' : minutesAgo < 60 ? `${minutesAgo} min` : `${Math.floor(minutesAgo / 60)}h${minutesAgo % 60}m`;
            const timeBg = minutesAgo <= 5 ? 'bg-emerald-50 text-emerald-700 border-emerald-200/60' : minutesAgo <= 15 ? 'bg-amber-50 text-amber-700 border-amber-200/60' : 'bg-rose-50 text-rose-700 border-rose-200/60';
            const stopCount = Array.isArray(corrida.paradas) ? corrida.paradas.length : 0;
            return (
            <div 
              key={corrida.id}
            ...
            >
              ...
              <div className="flex justify-between items-start mb-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={cn("font-bold tracking-tight text-sm", isSelected ? "text-white" : "text-zinc-900")}>#{corrida.codigoPedido}</span>
                    <span className={cn(
                      "px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border",
                      isSelected ? "bg-white/10 text-white border-white/10" : 
                      corrida.rawStatus === 'E' || corrida.rawStatus === 'A' ? "bg-emerald-50 text-emerald-700 border-emerald-200/60" : 
                      "bg-amber-50 text-amber-700 border-amber-200/60"
                    )}>
                      {corrida.status}
                    </span>
                  </div>
                  <p className={cn("text-[10px] mt-0.5 font-mono", isSelected ? "text-zinc-500" : "text-zinc-400")}>OS: #{corrida.id} • {corrida.tipo}</p>
                </div>
                <span className={cn(
                  "px-2 py-0.5 rounded-full text-[10px] font-bold border shrink-0 tabular-nums",
                  isSelected ? "bg-white/10 text-white border-white/10" : timeBg
                )}>
                  <Clock className="inline h-2.5 w-2.5 mr-0.5 -mt-0.5" />{timeLabel}
                </span>
              </div>

              {/* Info row: driver + client + value */}
              <div className={cn("flex items-center gap-3 text-[10px] mb-2.5", isSelected ? "text-zinc-400" : "text-zinc-500")}>
                <div className="flex items-center gap-1 min-w-0 flex-1">
                  <Truck strokeWidth={1.5} className="h-3 w-3 shrink-0" />
                  <span className="truncate">{corrida.motoboy?.nome || "Sem motorista"}</span>
                </div>
                <div className="flex items-center gap-1 min-w-0 flex-1">
                  <User strokeWidth={1.5} className="h-3 w-3 shrink-0" />
                  <span className="truncate">{corrida.cliente || "Cliente"}</span>
                </div>
                <span className={cn("font-mono font-bold text-[11px] shrink-0", isSelected ? "text-emerald-400" : "text-zinc-900")}>
              ...
              <div className={cn(
                ...
                isSelected ? "before:bg-zinc-700" : "before:bg-zinc-200"
              )}>
                <div className="relative">
                  <div className={cn("absolute -left-[14px] top-1.5 w-1.5 h-1.5 rounded-full", isSelected ? "bg-emerald-400" : "bg-emerald-500")} />
                  <div className="flex gap-2 items-start">
                    <span className={cn("text-[10px] font-mono mt-0.5 shrink-0 w-10", isSelected ? "text-zinc-500" : "text-zinc-400")}>{corrida.coleta?.hora || "--:--"}</span>
                    <p className={cn("text-[11px] font-medium line-clamp-1", isSelected ? "text-zinc-200" : "text-zinc-700")}>{corrida.coleta?.endereco || "Endereço não disponível"}</p>
                  </div>
                </div>
                {(corrida.paradas || []).slice(0, 3).map((p: any, idx: number) => (
                  <div className="relative" key={p.id || idx}>
                    <div className={cn("absolute -left-[14px] top-1.5 w-1.5 h-1.5 rounded-full border border-current", isSelected ? "text-zinc-500 bg-zinc-900" : "text-zinc-300 bg-white")} />
                    <div className="flex gap-2 items-start">
                      <span className={cn("text-[10px] font-mono mt-0.5 shrink-0 w-10", isSelected ? "text-zinc-600" : "text-zinc-400")}>{corrida.entrega?.hora || "--:--"}</span>
                      <p className={cn("text-[11px] line-clamp-1", isSelected ? "text-zinc-400" : "text-zinc-500")}>{p.endereco || "Destino"}{p.bairro ? `, ${p.bairro}` : ''}</p>
                    </div>
                  </div>
                ))}
                {stopCount > 3 && (
                  <div className={cn("text-[10px] pl-12 font-medium", isSelected ? "text-zinc-500" : "text-zinc-400")}>+{stopCount - 3} parada{stopCount - 3 > 1 ? 's' : ''}</div>
                )}
              </div>

              {/* Footer: distance + stops */}
              <div className={cn("flex items-center gap-3 mt-2.5 pt-2 border-t text-[10px]", isSelected ? "border-zinc-800 text-zinc-500" : "border-zinc-100 text-zinc-400")}>
                ...
                {stopCount > 0 && (
                  <div className="flex items-center gap-1">
                    <Navigation strokeWidth={1.5} className="h-2.5 w-2.5" />
                    <span>{stopCount} entrega{stopCount > 1 ? 's' : ''}</span>
                  </div>
                )}
                <div className="flex items-center gap-1">
                  <Store strokeWidth={1.5} className="h-2.5 w-2.5" />
                  <span className="truncate max-w-[100px]">{corrida.empresa}</span>
                </div>
              </div>
            </div>
            );
      ...
      <div id="main-content-area" className="flex-1 flex flex-col min-w-0 h-full bg-white relative">
        {/* Map Section */}
        <div className="flex-1 relative bg-zinc-100 border-b border-zinc-200 overflow-hidden min-h-[280px]">
            ...
            >
              ...
              <div className="absolute top-3 right-3 z-[400] flex flex-col items-end gap-2 pointer-events-none">
                <div className={cn(
                  ...
                  onlineCount > 0 ? "bg-emerald-600/90 border border-emerald-500/50" : "bg-blue-600/90 border border-blue-500/50"
                )}>
                  <span className="w-2 h-2 rounded-full bg-white animate-pulse" />
                  <span>{onlineCount > 0 ? `${onlineCount} motoboy${onlineCount > 1 ? 's' : ''} online` : 'Conectado • Nenhum motoboy ativo'}</span>
                </div>
              </div>
              <MapController 
                coords={
                  ...
                  : selectedCorrida?.motoboy?.id_num && posByDriverId.get(String(selectedCorrida.motoboy.id_num))
                    ? [
                        posByDriverId.get(String(selectedCorrida.motoboy.id_num))!.latitude,
                        posByDriverId.get(String(selectedCorrida.motoboy.id_num))!.longitude
                      ]
                    : selectedCorrida?.origin 
                      ? [selectedCorrida.origin.lat, selectedCorrida.origin.lng] 
                      : (storeLocation || null)
                } 
                rideId={focusDriverId ? `driver-${focusDriverId}` : (selectedCorrida?.id || 'store-initial')} 
                focusKey={focusKey}
              />
              {selectedCorrida?.origin && (
                <Marker position={[selectedCorrida.origin.lat, selectedCorrida.origin.lng]} icon={storeIcon}>
                  <Tooltip direction="top" offset={[0, -10]} opacity={0.95}>Loja (Coleta)</Tooltip>
                  <Popup>Coleta: {selectedCorrida.coleta?.endereco}</Popup>
                </Marker>
              )}
              {selfPosition && (
                <Marker position={selfPosition} icon={selfIcon} zIndexOffset={1100}>
                  <Tooltip direction="top" offset={[0, -10]} opacity={0.95}>Sua localização</Tooltip>
                </Marker>
              )}
              {storeLocation && (!selectedCorrida || (
                selectedCorrida.origin && (
                  Math.abs(selectedCorrida.origin.lat - storeLocation[0]) > 0.0001 || 
                  Math.abs(selectedCorrida.origin.lng - storeLocation[1]) > 0.0001
                )
              )) && (
                <Marker position={[storeLocation[0], storeLocation[1]]} icon={storeIcon}>
                  <Popup>Sua Loja: {companyName}</Popup>
                </Marker>
              )}
              {selectedCorrida?.destination && (
                <Marker position={[selectedCorrida.destination.lat, selectedCorrida.destination.lng]} icon={dropoffIcon}>
              ...
              {/* All active drivers */}
              {(Array.from(posByDriverId.values()) as DriverPosition[]).map((pos) => {
                const matchingRide = activeRides.find(r => String(r.motoboy?.id_num) === String(pos.machine_condutor_id));
                const directoryEntry = driverDirectory[String(pos.machine_condutor_id)];
                const driverName = String(matchingRide?.motoboy?.nome || directoryEntry?.name || '').trim() || `Motoboy ${pos.machine_condutor_id}`;
                const driverPhone = String(directoryEntry?.phone || '').trim();
                const driverStatus = String(directoryEntry?.status || '').trim();
                const isSelectedDriver = selectedDriverId && String(pos.machine_condutor_id) === selectedDriverId;
                      ...
                      <div className="p-1">
                        ...
                        <div className="space-y-1">
                          <p className="text-[10px] text-zinc-500 flex items-center gap-1">
                            <Clock className="h-3 w-3" /> Atualizado há {Math.max(0, Math.round((Date.now() - new Date(pos.received_at).getTime()) / 60000))} min
                          </p>
                          {driverPhone && (
          ...
          {!selectedCorrida ? (
            <div className="absolute top-3 left-1/2 -translate-x-1/2 pointer-events-none" style={{ zIndex: 400 }}>
              <div className="bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-md border border-zinc-200/60 flex items-center gap-2 text-xs font-medium text-zinc-500">
                ...
                Selecione uma corrida para ver os pontos de coleta e entrega no mapa
              </div>
            </div>
          ) : (
            <>
              <div className="absolute top-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-3xl pointer-events-none" style={{ zIndex: 400 }}>
                <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-lg border border-zinc-200/50 flex flex-wrap sm:flex-nowrap divide-y sm:divide-y-0 sm:divide-x divide-zinc-100 pointer-events-auto">
                  <div className="flex w-full sm:w-1/2 divide-x divide-zinc-100">
                    ...
                    />
                    <StatColumn icon={<MapPin className="h-4 w-4 text-zinc-400" />} label="Última parada" value={selectedCorrida.lastStop || "Nenhuma"} />
                  </div>
                  <div className="flex w-full sm:w-1/2 divide-x divide-zinc-100">
                    <StatColumn
                      icon={<ChevronRight className="h-4 w-4 text-zinc-400" />}
                      label={selfToPickupKm !== null ? "Você → Coleta" : "Distância"}
                      value={selfToPickupKm !== null ? `${selfToPickupKm.toFixed(2)} km` : (selectedCorrida.distancia || "0 km")}
                    />
                    <StatColumn 
              ...
              <div className="absolute bottom-4 left-4 right-4 pointer-events-none" style={{ zIndex: 400 }}>
                <div className="bg-white rounded-xl shadow-lg border border-zinc-200/80 overflow-hidden flex flex-col pointer-events-auto">
                  <div className="flex border-b border-zinc-100 bg-zinc-50/50 overflow-x-auto hide-scrollbar">
                    {['Detalhes do pedido', 'Informações do motorista', 'Veículo', 'Informações do cliente'].map(tab => (
                      <button 
                        key={tab}
                      ...
                      {activeTab === 'Informações do motorista' ? (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="h-12 w-12 rounded-full bg-zinc-100 border border-zinc-200 flex items-center justify-center text-zinc-800 font-bold text-xl shadow-sm shrink-0">
                              {selectedCorrida.motoboy?.nome ? selectedCorrida.motoboy.nome.charAt(0).toUpperCase() : "M"}
                            </div>
                            <div>
                              <h3 className="text-lg font-bold text-zinc-900 tracking-tight">{selectedCorrida.motoboy?.nome || "Sem motorista"}</h3>
                              <div className="flex items-center gap-1.5 mt-0.5">
                                {selectedCorrida.motoboy?.nome ? (
                                  <>
                                    <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" />
                                    <span className="text-xs font-medium text-zinc-500">Motorista Ativo</span>
                                  </>
                                ) : (
                                  <span className="text-xs font-medium text-zinc-400">Aguardando motorista</span>
                                )}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-x-8 gap-y-4 text-sm grow sm:justify-end border-l border-zinc-100 pl-6 ml-2">
                            <InfoBlock label="Experiência" value={selectedCorrida.motoboy?.exp || "-"} />
                            <InfoBlock label="CNH" value={selectedCorrida.motoboy?.cnh || "-"} />
                            <InfoBlock label="Documento" value={selectedCorrida.motoboy?.id_num || "-"} />
                          </div>

                          <div className="flex gap-2 shrink-0 w-full sm:w-auto">
                            <button 
                              onClick={() => {
                                const phone = driverDirectory[String(selectedCorrida.motoboy?.id_num)]?.phone || selectedCorrida.telefoneMotorista || '';
                                if (phone) {
                                  window.open(`tel:${phone.replace(/\D/g, '')}`, '_self');
                                } else {
                                  showToast(`Telefone de ${selectedCorrida.motoboy?.nome || 'Motorista'} não disponível.`);
                                }
                              }}
                              ...
                              <Phone className="h-3.5 w-3.5 fill-current" /> Ligar
                            </button>
                            <button 
                              onClick={() => {
                                const phone = driverDirectory[String(selectedCorrida.motoboy?.id_num)]?.phone || selectedCorrida.telefoneMotorista || '';
                                if (phone) {
                                  ...
                                  showToast(`Telefone de ${selectedCorrida.motoboy?.nome || 'Motorista'} não disponível para chat.`);
                                }
                              }}
                          ...
                          <div className="flex items-center gap-4">
                            <div>
                              <h3 className="text-lg font-bold text-zinc-900 tracking-tight">
                                ...
                                <span className="text-xs font-mono font-normal text-zinc-400 ml-2">OS: #{selectedCorrida.id}</span>
                              </h3>
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <Store className="h-3.5 w-3.5 text-zinc-400" />
                                <span className="text-xs font-medium text-zinc-500">{selectedCorrida.empresa || "-"}</span>
                              </div>
                            </div>
                          </div>
                          
                          <div className="flex flex-wrap gap-x-8 gap-y-4 text-sm grow sm:justify-end border-l border-zinc-100 pl-6 ml-2">
                            <InfoBlock label="Tipo" value={selectedCorrida.tipo || "-"} />
                            <InfoBlock label="Criado em" value={selectedCorrida.horario || "-"} />
                            <InfoBlock label="Valor" value={selectedCorrida.valor ? formatCurrency(selectedCorrida.valor) : "-"} />
                          </div>
                          <div className="flex gap-2 shrink-0 w-full sm:w-auto">
                            {['Concluída', 'Cancelada', 'Não atendida'].includes(selectedCorrida.status) ? (
                              ...
                              <>
                                <button
                                  onClick={() => handleOpenTracking(selectedCorrida.id)}
                                  disabled={loadingTracking === String(selectedCorrida.id)}
                                ...
                                >
                                  {loadingTracking === String(selectedCorrida.id) ? (
                                ...
                                <button
                                  ...
                                  try {
                                    const res = await authFetch('/api/machine/rides/cancel', { 
                                      ...
                                      body: JSON.stringify({ id_mch: selectedCorrida.id, motivo_id: 7 })
                                    });
                                    if (res.ok) {
                                      showToast(`Corrida #${selectedCorrida.id} cancelada com sucesso.`);
                                      const canceledRide = { ...selectedCorrida, status: "Cancelada" };
                                      setActiveRides(activeRides.filter(r => r.id !== selectedCorrida.id));
                                      setHistoryRides([canceledRide, ...historyRides]);
                                      setSelectedCorrida(null);
                                    } else {
                                      const d = await res.json().catch(() => ({}));
                                      const errMsg = d?.error || d?.details?.errors?.[0]?.message || 'Erro ao cancelar a corrida';
                                      showToast(`Erro: ${errMsg}`);
                                    }
                                  } catch (err) {
                                    showToast('Falha de conexão ao cancelar corrida.');
                                  }
                                }}
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                               <h3 className="text-lg font-bold text-zinc-900 tracking-tight">{selectedCorrida.motoboy?.veiculo || "Motocicleta Padrão"}</h3>
                               <div className="flex items-center gap-1.5 mt-0.5">
                                 <span className="text-xs font-medium text-zinc-500 uppercase tracking-widest">{selectedCorrida.motoboy?.placa || "XXX-0000"}</span>
                               </div>
                             </div>
                          </div>
                           <div className="flex flex-wrap gap-x-8 gap-y-4 text-sm grow sm:justify-end border-l border-zinc-100 pl-6 ml-2">
                        ...
                        >
                          <div className="flex items-center gap-4">
                             <div className="h-12 w-12 rounded-full bg-indigo-100 border border-indigo-200 flex items-center justify-center text-indigo-800 font-bold text-xl shadow-sm shrink-0">
                                {selectedCorrida.cliente ? selectedCorrida.cliente.charAt(0).toUpperCase() : "C"}
                             </div>
                             <div>
                               <h3 className="text-lg font-bold text-zinc-900 tracking-tight">{selectedCorrida.cliente || "Cliente"}</h3>
                               <div className="flex items-center gap-1.5 mt-0.5">
                                 <span className="text-xs font-medium text-zinc-500">Ver perfil completo</span>
                               </div>
                             </div>
                          </div>
                          <div className="flex flex-wrap gap-x-8 gap-y-4 text-sm grow sm:justify-end border-l border-zinc-100 pl-6 ml-2">
                             <InfoBlock label="Telefone" value={selectedCorrida.telefoneCliente || "Não informado"} />
                             <InfoBlock label="Pedidos" value="Pedido atual" />
                          </div>
                          <div className="flex gap-2 shrink-0 w-full sm:w-auto">
                            <button 
                              onClick={() => {
                                const phone = selectedCorrida.telefoneCliente || '';
                                if (phone) {
                                  window.open(`tel:${phone.replace(/\D/g, '')}`, '_self');
                                } else {
                                  showToast('Telefone do cliente não disponível.');
                                }
                              }}
                              className="flex-1 sm:flex-none flex justify-center items-center gap-2 px-4 py-2 bg-zinc-800 text-white rounded-lg hover:bg-zinc-900 text-sm font-semibold shadow-sm transition-all">
                              <Phone className="h-3.5 w-3.5 fill-current" /> Ligar para Cliente
                            </button>
                            <button 
                              onClick={() => {
                                const phone = selectedCorrida.telefoneCliente || '';
                                if (phone) {
                                  ...
                                  showToast('Telefone do cliente não disponível para chat.');
                                }
                              }}
                              className="flex-1 sm:flex-none flex justify-center items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 text-sm font-semibold shadow-sm transition-all">
                              <MessageSquare className="h-3.5 w-3.5" /> WhatsApp Cliente
                            </button>
                          </div>
                        </motion.div>
                      ) : null}
        ...
        >
          ...
          <div className="p-4 border-b border-zinc-200 flex flex-col sm:flex-row gap-3 items-center justify-between bg-zinc-50/50 shrink-0">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                {['Todas', 'Concluídas', 'Canceladas'].map(status => (
                  <button 
                    ...
                    className={cn(
                      ...
                      historyFilter === status ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700"
                    )}
                  >
                    {status}
                  </button>
              ...
              <button onClick={handleExportCSV} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-zinc-200 text-zinc-700 rounded-lg hover:bg-zinc-50 hover:text-zinc-900 text-xs font-medium transition-all shadow-sm">
                ...
                {error ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-red-500 bg-red-50/50">
                       <span className="font-bold">Erro ao carregar histórico:</span> {error}
                    </td>
                  </tr>
                ) : loading ? (
                  <tr>
                    ...
                    <td className="px-4 py-2">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className={cn(
                          ...
                          corrida.status === 'Em andamento' ? 'bg-blue-100 text-blue-700' :
                          corrida.status === 'Coletando' ? 'bg-amber-100 text-amber-700' :
                          corrida.status === 'Concluída' ? 'bg-emerald-100 text-emerald-700' :
                    ...
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        <div className="w-5 h-5 rounded-full bg-zinc-900 flex items-center justify-center text-white font-bold text-[9px] ring-2 ring-white shadow-sm">
                          {(corrida.motoboy?.nome || "Sem motorista").charAt(0)}
                        </div>
                        <span className="font-medium text-zinc-900 text-xs">{corrida.motoboy?.nome || "Sem motorista"}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2">
                      <div className="flex flex-col text-zinc-800">
                        <div className="flex items-center gap-1.5 font-medium text-xs">
                          <User strokeWidth={1.5} className="h-3 w-3 text-zinc-400 shrink-0" />
                          <span className="line-clamp-1">{corrida.cliente}</span>
                        </div>
                        {corrida.telefoneCliente && (
                          <span className="font-mono text-zinc-400 text-[10px] pl-4">{corrida.telefoneCliente}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-2">
                       <div className="flex items-center gap-1.5 text-zinc-600">
                        <Store strokeWidth={1.5} className="h-3 w-3 text-zinc-400" />
                        <span className="font-medium text-xs">{corrida.empresa}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-1.5 text-zinc-500">
                        <MapPin strokeWidth={1.5} className="h-3 w-3" />
                        <span className="font-mono text-[11px]">{corrida.distancia}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button 
                          ...
                          title="Ver no mapa"
                        >
                          <MapIcon strokeWidth={1.5} className="h-2.5 w-2.5 text-indigo-500" />
                          Trajeto
                        </button>
                        <button 
                          onClick={(e) => {
                        ...
                        {(corrida.rawStatus === 'F' || corrida.status === 'Concluída') && (
                          <button 
                            onClick={async (e) => {
                              e.stopPropagation();
                              try {
                                const res = await fetch(`/api/machine/rides/receipt?solicitacao_id=${corrida.id}`);
                                if (res.ok) {
                                  const data = await res.json();
                                  // Open receipt in new tab as formatted page
                                  const w = window.open('', '_blank');
                                  if (w) {
                                    w.document.write(`<html><head><title>Recibo #${corrida.id}</title><style>body{font-family:system-ui,-apple-system,sans-serif;padding:2rem;max-width:600px;margin:0 auto;color:#18181b}pre{white-space:pre-wrap;font-size:13px;background:#f4f4f5;padding:1.5rem;border-radius:12px;border:1px solid #e4e4e7}</style></head><body><h2>Recibo — Corrida #${corrida.id}</h2><pre>${JSON.stringify(data, null, 2)}</pre></body></html>`);
                                    w.document.close();
                                  }
                                } else {
                                  alert('Recibo não disponível para esta corrida.');
                                }
                              } catch {
                            ...
                            title="Ver recibo"
                          >
                            <FileText strokeWidth={1.5} className="h-2.5 w-2.5 text-emerald-500" />
                            Recibo
                          </button>
                        )}
                      </div>
          ...
          <div className="px-4 py-2.5 border-t border-zinc-200 bg-zinc-50/50 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0">
            <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider">
              Mostrando {Math.min(filteredHistoryRides.length, (currentPage - 1) * itemsPerPage + 1)}-{Math.min(filteredHistoryRides.length, currentPage * itemsPerPage)} de {filteredHistoryRides.length} corridas
            </span>
            <div className="flex gap-2">
              <button 
      ...
      <RideMapModal isOpen={!!selectedHistoryMap} onClose={() => setSelectedHistoryMap(null)} corrida={selectedHistoryMap} />

      <CreateRideModal 
        isOpen={isCreateModalOpen}
        onClose={() => { setIsCreateModalOpen(false); setRedistributeRideData(null); }}
        onSave={handleCreateRide}
        currentCompany={machineCompanyData ? { ...currentCompany, ...machineCompanyData } : currentCompany}
...
function StatColumn({ icon, label, value }: { icon: ReactNode, label: string, value: string }) {
  return (
