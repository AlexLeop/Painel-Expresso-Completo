...
function DeltaBadge({ value }: { value: number }) {
  if (!Number.isFinite(value)) return <span className="text-zinc-300 text-[10px]">—</span>;
  const up = value >= 0;
  return (
...
export function Historico() {
  ...
  const currentCompany = user?.companies?.find(c => Number(c.id) === companyId);
  ...
  const { data: snapsRaw, isLoading } = useApiQuery<any>(snapsKey, { refreshInterval: 300_000 });
  ...
  const filtered = useMemo(() =>
    snapshots
      .filter(s => (s.week_start || s.created_at || '').slice(0, 10) >= cutoffDate)
      .sort((a, b) => (a.week_start || a.created_at || '').localeCompare(b.week_start || b.created_at || ''))
  , [snapshots, cutoffDate]);

  // Dados para o gráfico de barras
  const chartData = useMemo(() =>
    filtered.map(s => ({
      semana: (s.week_start || s.created_at || '').slice(5, 10).replace('-', '/'),
  ...
  const totals = useMemo(() => {
    const total = filtered.reduce((s, snap) => s + Number(snap.total_liquido || snap.total_payout || 0), 0);
    const corridas = filtered.reduce((s, snap) => s + Number(snap.total_corridas || snap.rides_count || 0), 0);
    const motoboys = filtered.reduce((s, snap) => Math.max(s, Number(snap.total_motoboys || snap.drivers_count || 0)), 0);
  ...
  const deltaVsPrev = useMemo(() => {
    ...
    const prevTotal = prevSnaps.reduce((s, snap) => s + Number(snap.total_liquido || snap.total_payout || 0), 0);
  ...
  const exportSnapsCSV = () => {
    ...
    const rows = filtered.map(s => {
      ...
      const total = (Number(s.total_liquido || s.total_payout || 0)).toFixed(2).replace('.', ',');
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 pb-20 bg-[#F9F9FA]">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">{companyName} · Evolução mensal e semanal de faturamento</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total do Período", value: formatCurrency(totals.total), icon: <DollarSign className="h-4 w-4" />, color: "bg-emerald-100 text-emerald-700" },
          { label: "Média por Semana", value: formatCurrency(totals.avg), icon: <TrendingUp className="h-4 w-4" />, color: "bg-blue-100 text-blue-700" },
          { label: "Semanas Fechadas", value: totals.semanas, icon: <Calendar className="h-4 w-4" />, color: "bg-zinc-100 text-zinc-700" },
          { label: "Total de Corridas", value: totals.corridas.toLocaleString('pt-BR'), icon: <FileText className="h-4 w-4" />, color: "bg-orange-100 text-orange-700" },
        ].map((kpi, i) => (
          <div key={i} className="bg-white rounded-2xl p-4 shadow-sm ring-1 ring-zinc-200/60 flex items-center gap-3">
            <div className={cn("p-2.5 rounded-xl shrink-0", kpi.color)}>{kpi.icon}</div>
            <div>
              <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{kpi.label}</p>
              <p className="text-xl font-black text-zinc-900 tracking-tight">{kpi.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm ring-1 ring-zinc-200/60 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-bold text-zinc-900">Faturamento por Semana Fechada</h2>
            <p className="text-xs text-zinc-400 mt-0.5">{period === "3m" ? "Últimos 3 meses" : period === "6m" ? "Últimos 6 meses" : "Último ano"}</p>
          </div>
          {deltaVsPrev !== null && (
            <div className="flex items-center gap-2">
        ...
        {chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height={220} minWidth={1} minHeight={1}>
            <BarChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: -8 }} barSize={18}>
      ...
      <div className="bg-white rounded-2xl shadow-sm ring-1 ring-zinc-200/60 overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-100 flex items-center justify-between">
          <h2 className="text-sm font-bold text-zinc-900">Semanas Fechadas ({filtered.length})</h2>
        </div>
        {filtered.length === 0 ? (
          <div className="p-8 text-center text-zinc-400 text-sm">Nenhum fechamento encontrado no período.</div>
        ) : (
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-zinc-100">
                {[...filtered].reverse().map((s, i) => {
                  ...
                  const prevTotal = i < filtered.length - 1 ? Number(filtered[filtered.length - 2 - i]?.total_liquido || filtered[filtered.length - 2 - i]?.total_payout || 0) : null;
                    ...
                    <tr key={i} className="hover:bg-zinc-50/50 transition-colors">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-zinc-900 font-mono">{semana}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-center text-zinc-600">{s.total_motoboys || s.drivers_count || '—'}</td>
                      <td className="px-4 py-3 text-center text-zinc-600">{(s.total_corridas || s.rides_count || 0).toLocaleString('pt-BR')}</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-bold",
                          s.status === 'closed' || s.fechado ? "bg-zinc-100 text-zinc-600" : "bg-blue-100 text-blue-700")}>
                          {s.status === 'closed' || s.fechado ? "Fechado" : "Aberto"}
                        </span>
                      </td>
                    </tr>
                  );
