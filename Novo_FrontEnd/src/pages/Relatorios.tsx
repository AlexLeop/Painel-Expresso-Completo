...
import { Download, FileText, Calendar, RefreshCw, BarChart3, Calculator, TrendingUp, DollarSign, Bike, ShieldCheck, ChevronLeft, ChevronRight } from "lucide-react";
import { formatCurrency, cn } from "../lib/utils";
import { useAuth } from "../contexts/AuthContext";
import { getCompanyConfig, pullConfigFromSupabase } from "../services/company-config";
import { exportToCSV } from "../services/export-csv";
import { exportToPDF } from "../services/export-pdf";
import { getDailyEntriesForWeek, getManualEntriesForWeek, pullEntriesFromSupabase, getDriverDayAggregation } from "../services/entries-store";
...
export function Relatorios() {
  ...
  const currentCompany = user?.companies?.find((c: any) => Number(c.id) === companyId);
  ...
  const [storeConfig, setStoreConfig] = useState(() => (companyId ? getCompanyConfig(companyId, companyName) : null));
  const [scheduleDailyByDriver, setScheduleDailyByDriver] = useState<Record<string, Record<string, number>>>({});
  const [scheduleShiftsByDriver, setScheduleShiftsByDriver] = useState<Record<string, Record<string, Array<{ start: string; end: string; dailyRate?: number; label?: string; minGuaranteedOverride?: number | null }>>>>({});

  const isAdmin = user?.role === "admin" || user?.role === "administrador" || user?.role === "master";
    ...
    try {
      ...
      try {
        ...
        const filtered = allRides.filter((ride: any) => {
          const dateRaw = String(ride?.data_hora_solicitacao || ride?.data_hora_finalizacao || "");
          const { dateISO } = parseDateTime(dateRaw);
        ...
        if (reachedCap) {
          setRidesWarning(`Atenção: a busca atingiu o limite máximo de páginas. O relatório pode estar incompleto.`);
        }
      } catch (err) {
        logger.error("Erro ao buscar corridas da Machine:", err);
        setMachineRides([]);
        setRidesWarning("Erro ao buscar corridas da Machine. Verifique a integração e tente novamente.");
      }

      try {
        const cfgRes = await authFetch(`/api/db/configs?company_id=${companyId}`);
        const cfgData = cfgRes.ok ? await cfgRes.json() : null;
        const companyUUID = cfgData?.company_id ? String(cfgData.company_id) : null;

        const cdRes = await authFetch(`/api/db/company-drivers?company_id=${companyId}&active_only=0`);
        const cdRaw = cdRes.ok ? await cdRes.json() : [];
        const cdList: Array<{ driverUUID: string; driverId: string }> = (Array.isArray(cdRaw) ? cdRaw : []).map((d: any) => ({
        ...
        if (companyUUID) {
          const schedRes = await authFetch(`/api/schedules?company_id=${encodeURIComponent(companyUUID)}&week_start=${encodeURIComponent(weekPeriod.start)}`);
          ...
          const entries = schedules.flatMap((s: any) => Array.isArray(s?.schedule_entries) ? s.schedule_entries : []);

          const map: Record<string, Record<string, number>> = {};
          const shifts: Record<string, Record<string, Array<{ start: string; end: string; dailyRate?: number; label?: string; minGuaranteedOverride?: number | null }>>> = {};
          for (const e of entries) {
            const driverUUID = String(e.driver_id || "");
            const machineId = String((e?.driver as any)?.machine_condutor_id || uuidToMachine.get(driverUUID) || "");
            if (!machineId) continue;
            const date = String(e.entry_date || "");
            if (!date) continue;
            const val = Number(e.daily_rate || 0);
            if (!map[machineId]) map[machineId] = {};
            map[machineId][date] = (map[machineId][date] || 0) + (Number.isFinite(val) ? val : 0);
            ...
            if (shiftStart && shiftEnd) {
              ...
              shifts[machineId][date].push({ start: shiftStart, end: shiftEnd, dailyRate: val, label: String(e.shift_label || ""), minGuaranteedOverride: e.min_guaranteed_override != null ? Number(e.min_guaranteed_override) : null });
            }
          }
  ...
  const getGarantidoDiarioConfig = useCallback((cfg: any, dateIso: string): number => {
    if (!cfg) return 0;
    const faixas = Array.isArray(cfg.faixasHoras) ? cfg.faixasHoras : (Array.isArray(cfg.faixas_horas_config) ? cfg.faixas_horas_config : []);
  ...
  const reportData = useMemo(() => {
    ...
    const dailies = getDailyEntriesForWeek(companyId, weekPeriod.start, weekPeriod.end);
    const manuals = getManualEntriesForWeek(companyId, weekPeriod.start, weekPeriod.end);
    ...
    const showTxCol = viewMode === 'motoboy' ? false : (reportType === 'producao'
      ? includeTaxaCorridas
    ...
    const perDriverPerDay: Record<string, Record<string, { productionValue: number; deliveries: number; rides: Array<{ timeHHmm: string; fareValue: number; numDeliveries: number }> }>> = {};
    const manualRidesToInject = manuals
      .filter(m => {
        const isInjectable = m.type === 'corrida_manual' || (m.type === 'extra' && (m.description || '').match(/^(\d+)\s*(?:entrega|corrida)s?\s*-\s*/i));
      ...
      .map(m => {
        ...
        if (numDeliveries === 1) {
          const match = (m.description || '').match(/^(\d+)\s*(?:entrega|corrida)s?\s*-\s*/i) || (m.description || '').match(/(\d+)\s*(?:entrega|corrida)/i);
    ...
    for (const ride of combinedRides) {
      ...
      const dateRaw = String(ride?.data_hora_solicitacao || ride?.data_hora_finalizacao || ride?.data || '');
      ...
      if (!dateKey || dateKey < weekPeriod.start || dateKey > weekPeriod.end) continue;

      const rawFare = ride?.valor_corrida ?? ride?.valor ?? 0;
      const fareValue = typeof rawFare === 'string' ? parseFloat(rawFare.replace(',', '.')) : Number(rawFare || 0);
      const numDeliveries = Array.isArray(ride?.paradas) ? ride.paradas.length : 1;

      if (!perDriverPerDay[driverId]) perDriverPerDay[driverId] = {};
      if (!perDriverPerDay[driverId][dateKey]) perDriverPerDay[driverId][dateKey] = { productionValue: 0, deliveries: 0, rides: [] };
      ...
      perDriverPerDay[driverId][dateKey].rides.push({ timeHHmm: parsed.timeHHmm, fareValue: validFare, numDeliveries });

      if (!driverNames.has(driverId)) {
        driverNames.set(driverId, String(ride?.nome_condutor || `Condutor ${driverId}`));
      }
    }
    ...
    const faixaHoras = Array.isArray(storeConfig.faixasHoras) ? storeConfig.faixasHoras : [];
    ...
    function pickGarantidoPorHoras(driverId: string, dateISO: string) {
      ...
      const intervals: Array<{ start: number; end: number }> = shifts.map(s => {
      ...
      const sortedFaixas = [...faixaHoras].sort((a, b) => Number(a.horasMaximas) - Number(b.horasMaximas));
      const faixa = sortedFaixas.find(f => hours >= Number(f.horasMinimas) && hours <= Number(f.horasMaximas));
    ...
    function pickGarantidoForShift(startStr: string, endStr: string) {
      ...
      const sortedFaixas = [...faixaHoras].sort((a, b) => Number(a.horasMaximas) - Number(b.horasMaximas));
      const faixa = sortedFaixas.find(f => hours >= Number(f.horasMinimas) && hours <= Number(f.horasMaximas)) || sortedFaixas[sortedFaixas.length - 1];
      return faixa ? (Number(faixa.valor) || 0) : 0;
    }
    ...
    function resolveDiariaValue(diariaObj: any, dateStr: string): number {
      ...
      if (dayOfWeek === 0) return (diariaObj.sunday !== undefined && diariaObj.sunday !== null ? Number(diariaObj.sunday) : (diariaObj.weekday !== undefined && diariaObj.weekday !== null ? Number(diariaObj.weekday) : 0));
      if (dayOfWeek === 6) return (diariaObj.saturday !== undefined && diariaObj.saturday !== null ? Number(diariaObj.saturday) : (diariaObj.weekday !== undefined && diariaObj.weekday !== null ? Number(diariaObj.weekday) : 0));
      return (diariaObj.weekday !== undefined && diariaObj.weekday !== null ? Number(diariaObj.weekday) : 0);
    }

    const rows = Array.from(allDriverIds).map(driverId => {
      const name = driverNames.get(driverId) || `Condutor ${driverId}`;
      const days: Record<string, any> = {};
      const shiftsSummary: Record<string, { producaoReal: number; diaria: number; garantidoMinimo: number; ridesFee: number }> = {};
      ...
      for (const d of weekPeriod.dates) {
        ...
        const ridesFee = deliveries * (storeConfig.taxaCorridaPerEntrega || 1.6);

        const agg = getDriverDayAggregation(companyId, driverId, d.iso, undefined, viewMode);
        ...
        let netSelected = 0;
        const shiftsBreakdown: Array<{ name: string; producaoReal: number; entregas: number; diaria: number; garantidoMinimo: number; ridesFee: number }> = [];

        if (reportType === 'garantida_horas' || reportType === 'garantida') {
          const workedShifts = scheduleShiftsByDriver[driverId]?.[d.iso] || [];
          const configuredTurnos: any[] = (Array.isArray(storeConfig.turnos) && storeConfig.turnos.length > 0) ? storeConfig.turnos : [
            { id: 't1', nome: 'T1', startTime: '08:00', endTime: '15:59' },
            { id: 't2', nome: 'T2', startTime: '16:00', endTime: '23:59' }
          ];
          ...
          if (reportType === 'garantida_horas') {
            for (let idx = 0; idx < configuredTurnos.length; idx++) {
              ...
              const baseName = ct.nome || ct.label || (idx === 0 ? 'T1' : idx === 1 ? 'T2' : `T${idx+1}`);
              const shiftName = baseName.length > 3 && (baseName.toLowerCase().includes('almo') || baseName.toLowerCase().includes('t1') || baseName.toLowerCase().includes('turno 1')) ? 'T1' : (baseName.length > 3 && (baseName.toLowerCase().includes('jan') || baseName.toLowerCase().includes('t2') || baseName.toLowerCase().includes('turno 2')) ? 'T2' : baseName);
              const csm = timeToMinutes(ct.startTime);
              const cem = timeToMinutes(ct.endTime);

              const matchedWorked = workedShifts.find(w => {
                if (w.label && w.label.toLowerCase() === baseName.toLowerCase()) return true;
              ...
              const shiftStart = matchedWorked ? matchedWorked.start : ct.startTime;
              ...
              if (workedShifts.length === 1) {
                ...
                shiftRides = dayRidesList.filter(r => isTimeInShift(r.timeHHmm, shiftStart, shiftEnd));
              }

              const shiftProduction = shiftRides.reduce((s, r) => s + r.fareValue, 0);
              const shiftDeliveries = shiftRides.reduce((s, r) => s + (r.numDeliveries || 1), 0);
              const shiftRidesFee = shiftDeliveries * (storeConfig.taxaCorridaPerEntrega || 1.6);

              let shiftDiariaFixa = 0;
              let shiftGarantidoMinimo = 0;
              let shiftPayout = 0;

              if (matchedWorked || shiftProduction > 0 || diariaFromEntries > 0) {
                shiftDiariaFixa = resolveDiariaValue(storeConfig.diaria, d.iso);
                if (ct.diaria) {
                  shiftDiariaFixa = resolveDiariaValue(ct.diaria, d.iso);
                }
                if (matchedWorked && matchedWorked.dailyRate != null && Number.isFinite(matchedWorked.dailyRate)) {
                  shiftDiariaFixa = matchedWorked.dailyRate;
                } else if (diariaFromEntries > 0 && workedShifts.length <= 1) {
                  shiftDiariaFixa = diariaFromEntries;
                }

                if (matchedWorked) {
                  shiftGarantidoMinimo = (matchedWorked.minGuaranteedOverride != null && Number.isFinite(matchedWorked.minGuaranteedOverride)) ? matchedWorked.minGuaranteedOverride : (pickGarantidoForShift(shiftStart, shiftEnd) || pickGarantidoPorHoras(driverId, d.iso) || shiftDiariaFixa);
                } else {
                  shiftGarantidoMinimo = 0;
                }

                shiftPayout = Math.max(shiftProduction + shiftDiariaFixa, shiftGarantidoMinimo);
              ...
              if (!shiftsSummary[shiftName]) {
                shiftsSummary[shiftName] = { producaoReal: 0, diaria: 0, garantidoMinimo: 0, ridesFee: 0 };
              }
              shiftsSummary[shiftName].producaoReal += shiftProduction;
              shiftsSummary[shiftName].diaria += shiftDiariaFixa;
              shiftsSummary[shiftName].garantidoMinimo += shiftGarantidoMinimo;
            ...
            if (workedShifts.length > 0) {
              for (const w of workedShifts) {
                const shiftRides = dayRidesList.filter(r => isTimeInShift(r.timeHHmm, w.start, w.end));
                const shiftProduction = shiftRides.reduce((s, r) => s + r.fareValue, 0);

                let shiftDiariaFixa = resolveDiariaValue(storeConfig.diaria, d.iso);
                ...
                if (matchedTurno && matchedTurno.diaria) {
                  shiftDiariaFixa = resolveDiariaValue(matchedTurno.diaria, d.iso);
                }
                ...
                const shiftGarantidoMinimo = getGarantidoDiarioConfig(storeConfig, d.iso);
                const shiftPayout = Math.max(shiftProduction + shiftDiariaFixa, shiftGarantidoMinimo);

                dayDiariaFixa += shiftDiariaFixa;
                dayGarantidoMinimo += shiftGarantidoMinimo;
                dayPayout += shiftPayout;
              }
            } else if (productionValue > 0 || diariaFromEntries > 0 || scheduleDailyByDriver[driverId]?.[d.iso] !== undefined) {
              dayDiariaFixa = diariaFromEntries > 0 ? diariaFromEntries : (scheduleDailyByDriver[driverId]?.[d.iso] ?? resolveDiariaValue(storeConfig.diaria, d.iso));
              dayGarantidoMinimo = getGarantidoDiarioConfig(storeConfig, d.iso);
              dayPayout = Math.max(productionValue + dayDiariaFixa, dayGarantidoMinimo);
            }
          }
          ...
          // reportType === 'producao'
          const diariaFromSchedule = scheduleDailyByDriver[driverId]?.[d.iso] || 0;
          dayDiariaFixa = diariaFromEntries > 0 ? diariaFromEntries : diariaFromSchedule;
        ...
        totalLiquido += viewMode === 'motoboy' ? netSelected : (netSelected + ridesFee);
      }
    ...
    const pisoByPerc = storeConfig.pisoPercentual > 0 ? totalMotoboys * (storeConfig.pisoPercentual / 100) : 0;
    ...
    const totalALiquidar = totalMotoboys + txAdm + txSupervisao + debitoPendente;

    const totalShiftsSummary: Record<string, { producaoReal: number; diaria: number; garantidoMinimo: number; ridesFee: number }> = {};
    for (const r of rows) {
      if (r.shiftsSummary) {
        for (const [sName, sVals] of Object.entries(r.shiftsSummary)) {
          if (!totalShiftsSummary[sName]) totalShiftsSummary[sName] = { producaoReal: 0, diaria: 0, garantidoMinimo: 0, ridesFee: 0 };
  ...
  }, [companyId, storeConfig, weekPeriod, machineRides, syncVersion, scheduleDailyByDriver, scheduleShiftsByDriver, viewMode]);

  if (!reportData) return <div className="p-8 text-center text-zinc-500">Carregando dados do relatório...</div>;
      ...
      <div className="space-y-6 max-w-[1400px] mx-auto pb-20">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-6 sm:p-8 rounded-2xl shadow-lg border border-blue-700/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-sm font-semibold opacity-90">Período: {weekPeriod.label}</span>
            </div>
          </div>
          <div className="text-left sm:text-right relative z-10">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80 mb-2">Status do Relatório</p>
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 px-4 py-2 rounded-full shadow-sm">
              ...
              <p className="text-xs font-bold uppercase tracking-wider text-emerald-50">
                {reportData.reportType === 'producao'
                  ...
                  : (reportData.reportType === 'garantida_horas' ? 'Garantida por Horas' : 'Garantida Mínima')}
              </p>
            </div>
          </div>
        </div>
        ...
        {viewMode === 'loja' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <StatCard label="Logística Total" value={reportData.totals.logistica} color="blue" />
            <StatCard label="Taxa ADM (Corridas)" value={reportData.totals.taxaAdm} color="orange" sub="Piso mínimo" />
            <StatCard label="Total Corridas" value={reportData.totals.taxaCorridas} color="purple" sub={`R$ ${(storeConfig?.taxaCorridaPerEntrega || 1.6).toFixed(2)}/entrega`} />
            <StatCard label="Taxa Supervisão" value={reportData.totals.supervisao} color="green" />
            <StatCard label="Débito Pendente" value={reportData.totals.debito} color="red" />
          </div>
        )}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between bg-white px-4 py-3 rounded-2xl shadow-sm ring-1 ring-zinc-200/60 gap-4 mb-4">
            ...
            <div className="flex bg-zinc-100 p-1 rounded-lg border border-zinc-200 mr-2">
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${
                  viewMode === 'loja' ? 'bg-white text-zinc-900 shadow-sm ring-1 ring-zinc-200' : 'text-zinc-500 hover:text-zinc-700'
                }`}
              >
                Visão Loja
              </button>
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${
                  viewMode === 'motoboy' ? 'bg-white text-zinc-900 shadow-sm ring-1 ring-zinc-200' : 'text-zinc-500 hover:text-zinc-700'
                }`}
              >
                Visão Motoboy
              </button>
              ...
              exportToCSV({
                rows: reportData.drivers.map((d: any) => ({
                  ...
                  dias: Object.fromEntries(
                    Object.entries(d.days).map(([iso, day]: [string, any]) => [
                      iso,
                      {
                ...
                weekDates: weekPeriod.dates.map(d => ({ iso: d.iso, dayName: d.label })),
          ...
          <button
            onClick={() => {
              ...
              exportToPDF({
                ...
                weekDates: weekPeriod.dates.map(d => ({ iso: d.iso, dayName: d.label, fullLabel: d.fullLabel })),
                reportType: reportData.reportType as any,
                includeTaxaCorridas: reportData.showTxCol,
                companyName: companyName,
                periodLabel: weekPeriod.label,
        ...
        <div className="flex items-center gap-4 bg-zinc-50 px-2 py-1 rounded-xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-zinc-700 tracking-wide">{weekPeriod.label}</span>
          </div>
          <button 
            onClick={() => setCurrentWeekOffset(o => o + 1)}
            className="p-2 hover:bg-white hover:shadow-sm rounded-lg transition-all text-zinc-500 hover:text-zinc-900 focus:ring-2 focus:ring-zinc-200 outline-none"
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-max text-left whitespace-nowrap text-[11px] sm:text-xs">
            <thead className="bg-zinc-50/80 border-b border-zinc-200 text-zinc-500 font-bold uppercase tracking-wider">
              <tr>
                <th className="px-3 py-2.5 sticky left-0 bg-zinc-50 z-10 border-r border-zinc-100">Motoboy</th>
                ...
                <th className="px-4 py-2.5 text-right bg-zinc-900 text-white font-black border-l border-zinc-800 sticky right-0 z-20 shadow-[-4px_0_12px_rgba(0,0,0,0.1)]">Total Líquido</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 font-bold">
              {reportData.drivers.map((mb, i) => (
                <tr key={i} className="hover:bg-zinc-50/50 transition-colors group">
                  <td className="px-3 py-1.5 sticky left-0 bg-white group-hover:bg-zinc-50 z-10 border-r border-zinc-100 text-zinc-900">{mb.name}</td>
                  {weekPeriod.dates.map(d => {
                    const day = mb.days[d.iso];
                    return (
                      <td key={d.iso} className="px-2 py-1.5 text-center align-middle">
                        {day && (day.producaoReal > 0 || day.entregas > 0 || day.diaria > 0 || day.extras > 0 || (day.shifts && day.shifts.some((s: any) => s.producaoReal > 0 || s.diaria > 0 || s.garantidoMinimo > 0))) ? (
                          reportData.reportType === 'garantida_horas' && day.shifts && day.shifts.length > 0 ? (
                              ...
                              <table className="mx-auto text-[10px] sm:text-[11px] font-mono font-bold text-emerald-600 leading-tight">
                                <tbody>
                                  {day.shifts.map((s: any, sIdx: number) => (
                                    <tr key={sIdx}>
                                      <td className="text-right pr-1 font-semibold text-emerald-700">{s.name}:</td>
                                      <td className="text-left font-normal text-[10px] opacity-75 pr-0.5 align-baseline">R$</td>
                                      <td className="text-right align-baseline">{formatCurrency(s.producaoReal).replace("R$", "").replace(/\s/g, "").trim()}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                              {reportData.showEntregas && day.shifts.some((s: any) => s.entregas > 0) && (
                            ...
                            <div className="flex flex-col items-center leading-tight py-1">
                              ...
                              {reportData.showEntregas && day.entregas > 0 && (
                                <span className="text-[9px] text-zinc-400 uppercase tracking-tighter mt-0.5">{day.entregas} ents.</span>
                              )}
                            </div>
                          )
                  ...
                  {reportData.reportType === 'producao' ? (
                    <>
                      <td className="px-3 py-1.5 text-right font-mono text-zinc-600 align-middle">{formatCurrency(mb.totalDiaria)}</td>
                      <td className="px-3 py-1.5 text-right font-mono text-emerald-600 align-middle">{formatCurrency(mb.totalTaxa)}</td>
                    </>
                  ) : (
                    <>
                      <td className="px-3 py-1.5 text-right font-mono align-middle font-semibold text-zinc-900 text-xs">
                        {reportData.reportType === 'garantida_horas' && mb.shiftsSummary && Object.keys(mb.shiftsSummary).length > 0 ? (
                          <table className="ml-auto text-[10px] sm:text-[11px] font-mono font-semibold text-zinc-900 leading-tight">
                            <tbody>
                              {Object.entries(mb.shiftsSummary).map(([sName, sVals]: [string, any]) => (
                                <tr key={sName}>
                                  <td className="text-right pr-1 text-zinc-500 font-normal">{sName}:</td>
                                  <td className="text-left font-normal text-[10px] opacity-60 pr-0.5 align-baseline">R$</td>
                                  <td className="text-right align-baseline">{formatCurrency(sVals.producaoReal).replace("R$", "").replace(/\s/g, "").trim()}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                          ...
                          {reportData.reportType === 'garantida_horas' && mb.shiftsSummary && Object.keys(mb.shiftsSummary).length > 0 ? (
                            <table className="ml-auto text-[10px] sm:text-[11px] font-mono font-semibold text-orange-600 leading-tight">
                              <tbody>
                                {Object.entries(mb.shiftsSummary).map(([sName, sVals]: [string, any]) => (
                                  <tr key={sName}>
                                    <td className="text-right pr-1 text-orange-400 font-normal">{sName}:</td>
                                    <td className="text-left font-normal text-[10px] opacity-60 pr-0.5 align-baseline">R$</td>
                                    <td className="text-right align-baseline">{formatCurrency(sVals.diaria).replace("R$", "").replace(/\s/g, "").trim()}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                        ...
                        {reportData.reportType === 'garantida_horas' && mb.shiftsSummary && Object.keys(mb.shiftsSummary).length > 0 ? (
                          <table className="ml-auto text-[10px] font-mono font-semibold text-indigo-600 leading-tight">
                            <tbody>
                              {Object.entries(mb.shiftsSummary).map(([sName, sVals]: [string, any]) => (
                                <tr key={sName}>
                                  <td className="text-right pr-1 text-indigo-400 font-normal">{sName}:</td>
                                  <td className="text-left font-normal text-[10px] opacity-60 pr-0.5 align-baseline">R$</td>
                                  <td className="text-right align-baseline">{formatCurrency(sVals.garantidoMinimo).replace("R$", "").replace(/\s/g, "").trim()}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                      ...
                      {reportData.reportType === 'garantida_horas' && mb.shiftsSummary && Object.keys(mb.shiftsSummary).length > 0 ? (
                        <table className="ml-auto text-xs font-mono italic text-zinc-500 leading-tight">
                          <tbody>
                            {Object.entries(mb.shiftsSummary).map(([sName, sVals]: [string, any]) => (
                              <tr key={sName}>
                                <td className="text-right pr-1 text-zinc-400 font-normal">{sName}:</td>
                                <td className="text-left font-normal text-[10px] opacity-60 pr-0.5 align-baseline">R$</td>
                                <td className="text-right align-baseline">{formatCurrency(sVals.ridesFee).replace("R$", "").replace(/\s/g, "").trim()}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                  ...
                  <td className="px-3 py-1.5 text-right text-rose-500 font-mono align-middle">{mb.totalAdiantamentos > 0 ? `-${formatCurrency(mb.totalAdiantamentos)}` : "—"}</td>
            ...
            <tfoot className="bg-zinc-900 border-t-2 border-zinc-800 font-black text-white">
              <tr>
                <td className="px-3 py-2.5 sticky left-0 bg-zinc-900 z-10 text-[10px] uppercase tracking-widest border-r border-zinc-800 align-middle">Total Acumulado</td>
                {weekPeriod.dates.map(d => (
                  <td key={d.iso} className="px-2 py-2.5 text-center align-middle">—</td>
                ))}
                {reportData.reportType === 'producao' ? (
                  <>
                    <td className="px-3 py-2.5 text-right font-mono align-middle">{formatCurrency(reportData.drivers.reduce((s, d) => s + d.totalDiaria, 0))}</td>
                    <td className="px-3 py-2.5 text-right font-mono align-middle">{formatCurrency(reportData.drivers.reduce((s, d) => s + d.totalTaxa, 0))}</td>
                  </>
                ) : (
                  <>
                    <td className="px-3 py-2.5 text-right font-mono align-middle text-xs">
                      {reportData.reportType === 'garantida_horas' && reportData.totalShiftsSummary && Object.keys(reportData.totalShiftsSummary).length > 0 ? (
                        <table className="ml-auto text-[10px] sm:text-[11px] font-mono text-white leading-tight font-bold">
                          <tbody>
                            {Object.entries(reportData.totalShiftsSummary).map(([sName, sVals]: [string, any]) => (
                              <tr key={sName}>
                                <td className="text-right pr-1 text-zinc-400 font-normal">{sName}:</td>
                                <td className="text-left font-normal text-[10px] opacity-60 pr-0.5 align-baseline text-zinc-400">R$</td>
                                <td className="text-right align-baseline">{formatCurrency(sVals.producaoReal).replace("R$", "").replace(/\s/g, "").trim()}</td>
                              </tr>
                            ))}
                          </tbody>
                        ...
                        <span>{formatCurrency(reportData.drivers.reduce((s, d) => s + d.totalProducaoReal, 0))}</span>
                      )}
                    </td>
                    {reportData.showDiaria && (
                        ...
                        {reportData.reportType === 'garantida_horas' && reportData.totalShiftsSummary && Object.keys(reportData.totalShiftsSummary).length > 0 ? (
                          <table className="ml-auto text-[10px] sm:text-[11px] font-mono text-orange-400 leading-tight font-bold">
                            <tbody>
                              {Object.entries(reportData.totalShiftsSummary).map(([sName, sVals]: [string, any]) => (
                                <tr key={sName}>
                                  <td className="text-right pr-1 text-orange-300/70 font-normal">{sName}:</td>
                                  <td className="text-left font-normal text-[10px] opacity-60 pr-0.5 align-baseline text-orange-300/70">R$</td>
                                  <td className="text-right align-baseline">{formatCurrency(sVals.diaria).replace("R$", "").replace(/\s/g, "").trim()}</td>
                                </tr>
                              ))}
                            </tbody>
                          ...
                          <span>{formatCurrency(reportData.drivers.reduce((s, d) => s + d.totalDiaria, 0))}</span>
                        )}
                      </td>
                    )}
                      ...
                      {reportData.reportType === 'garantida_horas' && reportData.totalShiftsSummary && Object.keys(reportData.totalShiftsSummary).length > 0 ? (
                        <table className="ml-auto text-[10px] sm:text-[11px] font-mono text-indigo-400 leading-tight font-bold">
                          <tbody>
                            {Object.entries(reportData.totalShiftsSummary).map(([sName, sVals]: [string, any]) => (
                              <tr key={sName}>
                                <td className="text-right pr-1 text-indigo-300/70 font-normal">{sName}:</td>
                                <td className="text-left font-normal text-[10px] opacity-60 pr-0.5 align-baseline text-indigo-300/70">R$</td>
                                <td className="text-right align-baseline">{formatCurrency(sVals.garantidoMinimo).replace("R$", "").replace(/\s/g, "").trim()}</td>
                              </tr>
                            ))}
                          </tbody>
                        ...
                        <span>{formatCurrency(reportData.drivers.reduce((s, d) => s + d.totalGarantido, 0))}</span>
                      )}
                    </td>
                  </>
                )}
                {reportData.showTxCol && (
                  <td className="px-3 py-2.5 text-right font-mono align-middle text-zinc-300 text-xs">
                    {reportData.reportType === 'garantida_horas' && reportData.totalShiftsSummary && Object.keys(reportData.totalShiftsSummary).length > 0 ? (
                      ...
                      <span>{formatCurrency(reportData.totals.taxaCorridas)}</span>
                    )}
                  </td>
                )}
                <td className="px-3 py-2.5 text-right font-mono text-zinc-400 align-middle">-{formatCurrency(reportData.drivers.reduce((s, d) => s + d.totalAdiantamentos, 0))}</td>
                <td className="px-4 py-2.5 text-right font-mono text-[15px] border-l border-zinc-800 bg-zinc-900 text-white align-middle sticky right-0 z-20 shadow-[-4px_0_12px_rgba(0,0,0,0.1)]">{formatCurrency(reportData.totals.logistica)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-2xl shadow-sm ring-1 ring-zinc-200/60 gap-6 relative overflow-hidden mt-2">
          ...
          <div className="relative z-10 w-full md:w-auto">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-2xl font-black text-zinc-900 uppercase tracking-tight">Total a Liquidar (Loja)</h3>
            </div>
            <p className="text-xs text-zinc-500 font-medium ml-14">Tabela + TX Adm + TX Supervisão + Débito Pendente</p>
            <div className="mt-6 ml-14 flex flex-col gap-2.5 text-xs font-bold text-zinc-500 uppercase tracking-widest">
              <div className="flex justify-between w-64 items-center"><span className="opacity-70">Logística:</span> <span className="text-zinc-900">{formatCurrency(reportData.totals.logistica)}</span></div>
              {reportData.totals.taxaAdm > 0 && (
                <div className="flex justify-between w-64 items-center"><span className="opacity-70">+ Taxa ADM:</span> <span className="text-zinc-900">{formatCurrency(reportData.totals.taxaAdm)}</span></div>
              )}
              {reportData.totals.supervisao > 0 && (
                <div className="flex justify-between w-64 items-center"><span className="opacity-70">+ Supervisão:</span> <span className="text-emerald-600">{formatCurrency(reportData.totals.supervisao)}</span></div>
              )}
              {reportData.totals.debito > 0 && (
                <div className="flex justify-between w-64 items-center"><span className="opacity-70">+ Débito:</span> <span className="text-rose-600">{formatCurrency(reportData.totals.debito)}</span></div>
              )}
            </div>
          </div>
          <div className="relative z-10 w-full md:w-auto text-left md:text-right">
            <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-1.5">Valor Final</p>
...
function StatCard({ label, value, color, sub }: { label: string; value: number; color: "blue" | "orange" | "purple" | "green" | "red"; sub?: string }) {
  const colorMap = {
    blue: "text-blue-600",
  ...
  return (
    <div className="px-5 py-4 rounded-2xl bg-white shadow-sm ring-1 ring-zinc-200/60 transition-shadow hover:shadow-md flex flex-col justify-center">
      ...
      <p className={cn("text-xl sm:text-2xl font-black tracking-tight mt-1", colorMap[color])}>
        {formatCurrency(value)}
      </p>
      {sub && <p className="text-[10px] text-zinc-400 font-bold uppercase mt-1 tracking-wide">{sub}</p>}
    </div>
  );
}
...
function resolveBusinessDateISO(args: {
  ...
  rideDateISO: string;
  rideTimeHHmm: string;
  shiftsByDriver: Record<string, Record<string, Array<{ start: string; end: string }>>>;
  cutoffHHmm: string;
}) {
  const { driverId, rideDateISO, rideTimeHHmm, shiftsByDriver, cutoffHHmm } = args;
  const cutoffMinutes = timeToMinutes(cutoffHHmm);
  const shiftsToday = shiftsByDriver[driverId]?.[rideDateISO] || [];
  if (shiftsToday.some(s => isTimeInShift(rideTimeHHmm, s.start, s.end))) return rideDateISO;

  const prev = addDaysISO(rideDateISO, -1);
  const shiftsPrev = shiftsByDriver[driverId]?.[prev] || [];
  if (shiftsPrev.some(s => isOvernightShift(s.start, s.end) && isTimeInShift(rideTimeHHmm, s.start, s.end))) return prev;
  const prevQualifies = shiftsPrev.some(s => isOvernightShift(s.start, s.end) || timeToMinutes(s.end) <= cutoffMinutes);
