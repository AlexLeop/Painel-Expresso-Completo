...
import { Plus, Search, Filter, CalendarDays, ArrowUpRight, ArrowDownRight, MoreVertical, FileText, Edit2, Trash2, Loader2 } from "lucide-react";
...
export function Lancamentos() {
  ...
  const [lancamentoToEdit, setLancamentoToEdit] = useState<LancamentoType | null>(null);
  const [defaultCategoria, setDefaultCategoria] = useState<"Crédito" | "Débito">("Débito");
  ...
  const isAdmin = session?.user?.role === "admin" || session?.user?.role === "administrador" || session?.user?.role === "master";
  ...
  const fetchLancamentos = async () => {
    ...
    try {
      ...
      const [res, driversRes] = await Promise.all([
        ...
        authFetch(`/api/db/company-drivers?company_id=${companyId}&active_only=0`),
      ]);

      if (driversRes.ok) {
        const dData = await driversRes.json();
        const rawDrivers = Array.isArray(dData) ? dData : dData.motoboys || [];
        setMotoboys(rawDrivers.map((d: any) => ({
      ...
      if (res.ok) {
        ...
        const fetchedLancamentos = rawList.map((e: any) => ({
          id: e.id,
          motoboy: e.driverName || e.motoboy || '',
          tipo: e.type || e.tipo || '',
          categoria: (e.type === 'adiantamento' || e.categoria === 'Débito') ? 'Débito' : 'Crédito',
          valor: Number(e.amount ?? e.valor ?? 0),
          data: e.date ? e.date.split('T')[0].split('-').reverse().join('/') : (e.data || ''),
  ...
  const totalCreditos = lancamentos.filter(l => l.categoria === "Crédito").reduce((acc, curr) => acc + curr.valor, 0);
  const totalDebitos = lancamentos.filter(l => l.categoria === "Débito").reduce((acc, curr) => acc + curr.valor, 0);

  const handleOpenModal = (lancamento?: LancamentoType, categoria: "Crédito" | "Débito" = "Débito") => {
  ...
  const handleSaveLancamento = async (lancamento: LancamentoType) => {
    try {
      ...
      const payload = {
        ...
        type: lancamento.categoria === "Débito" ? "adiantamento" : (["extra", "corrida_manual"].includes(lancamento.tipo) ? lancamento.tipo : "extra"),
        amount: Number(lancamento.valor),
        date: isoDate,
        description: (lancamento.descricao || lancamento.tipo || (lancamento.categoria === "Débito" ? "Vale" : "Bônus")) + (lancamento.visibilidade && lancamento.visibilidade !== 'ambos' ? `|vis:${lancamento.visibilidade}` : '') + (lancamento.tipo === 'corrida_manual' && lancamento.entregas && lancamento.entregas > 1 ? `|entregas:${lancamento.entregas}` : '')
      };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-xl shadow-sm ring-1 ring-zinc-950/5">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 tracking-tight">Lançamentos Manuais</h1>
          <p className="text-sm text-zinc-500 mt-1">Gestão de vales, bônus e adiantamentos de motoboys.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-5">
            ...
            <h3 className="text-2xl font-bold text-zinc-900 tracking-tight pl-11">{formatCurrency(totalCreditos)}</h3>
         </div>
         <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-5">
            ...
            <h3 className="text-2xl font-bold text-zinc-900 tracking-tight pl-11">{formatCurrency(totalDebitos)}</h3>
         </div>
         <div className="bg-white rounded-xl shadow-sm border border-zinc-200 p-5">
            ...
            <h3 className="text-2xl font-bold text-zinc-900 tracking-tight pl-11">{lancamentos.length}</h3>
         </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-[0_1px_3px_0_rgba(0,0,0,0.05),_0_0_0_1px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-zinc-100">
              {error && (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-red-500 bg-red-50/50">
              ...
              {filteredLancamentos.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((lancamento) => (
                <motion.tr key={lancamento.id} variants={itemVariants} className="hover:bg-zinc-50/50 transition-colors group">
                  <td className="px-4 py-2">
                    <span className="font-mono text-[11px] text-zinc-600">{lancamento.data}</span>
                  </td>
                  <td className="px-4 py-2">
                    <span className="font-semibold text-zinc-900 text-xs">{lancamento.motoboy}</span>
                  </td>
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-2">
                       <span className={cn(
                          "inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border",
                          lancamento.categoria === 'Crédito' ? 'bg-emerald-50 text-emerald-700 border-emerald-200/60' : 'bg-rose-50 text-rose-700 border-rose-200/60'
                        )}>
                          {lancamento.categoria}
                        </span>
                        <span className="text-zinc-500 text-[11px] font-medium">{lancamento.tipo}</span>
                    </div>
                  </td>
                  <td className="px-4 py-2 text-zinc-600">
                    <span className="truncate max-w-[200px] block text-xs">{lancamento.descricao}</span>
                  </td>
                  <td className="px-4 py-2 text-right">
                    <span className={cn(
                      "font-mono font-bold text-xs tracking-tight",
                      lancamento.categoria === 'Crédito' ? 'text-emerald-600' : 'text-rose-600'
                    )}>
                      {lancamento.categoria === 'Crédito' ? '+' : '-'}{formatCurrency(lancamento.valor)}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      <button 
                        onClick={() => lancamento.id && handleDeleteLancamento(lancamento.id)}
        ...
        <div className="px-4 py-3 border-t border-zinc-200 bg-zinc-50 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider">
            Mostrando {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredLancamentos.length)} de {filteredLancamentos.length} registros
          </span>
          <div className="flex gap-2">
            <button 
            ...
            <button 
              ...
              disabled={currentPage * itemsPerPage >= filteredLancamentos.length}
              className="px-3 py-1.5 border border-zinc-200 rounded-lg bg-white text-zinc-700 hover:bg-zinc-50 disabled:opacity-50 disabled:text-zinc-400 text-[10px] font-bold uppercase tracking-wider shadow-sm transition-all"
            >
              Próxima
            </button>
