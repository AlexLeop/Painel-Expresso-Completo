...
export async function pullConfigFromSupabase(
  ...
  try {
    ...
    const config: CompanyConfig = {
      ...
      debitoPendente: parseVal(data.debito_pendente, 0),
      diaria: {
        weekday: parseVal(data.daily_rate_weekday, 60),
        saturday: parseVal(data.daily_rate_saturday, 70),
        sunday: parseVal(data.daily_rate_sunday, 80),
