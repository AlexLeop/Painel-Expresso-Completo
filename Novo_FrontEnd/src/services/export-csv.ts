...
export function exportToCSV(options: ExportOptions) {
  ...
  const dataRows = rows.map((row) => {
    ...
    weekDates.forEach((d) => {
      ...
      if (
        !day ||
        (day.producaoReal === 0 && day.entregas === 0 && day.diaria === 0)
      ) {
        vals.push("-");
    ...
    vals.push(
      row.adiantamentos > 0 ? `-${formatNumber(row.adiantamentos)}` : "-",
    );
  ...
  if (reportType === "producao") {
    ...
    totalRow.push(
      formatNumber(rows.reduce((s, r) => s + r.totalProducaoReal, 0)),
    );
    totalRow.push(formatNumber(rows.reduce((s, r) => s + r.payoutTotal, 0)));
  }
  if (includeTaxaCorridas)
    totalRow.push(formatNumber(rows.reduce((s, r) => s + r.taxaCorridas, 0)));
  totalRow.push(
    `-${formatNumber(rows.reduce((s, r) => s + r.adiantamentos, 0))}`,
  );
