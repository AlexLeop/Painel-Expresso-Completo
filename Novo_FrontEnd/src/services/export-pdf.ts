...
export function exportToPDF(options: ExportPDFOptions): void {
  try {
    ...
    const effectiveTotalALiquidar =
      totalALiquidar ?? totalGeral + txAdm + txSupervisao + debitoPendente;
    ...
    const txCol = includeTaxaCorridas
      ? [{ content: "TX CORR.", styles: { halign: "right" as const } }]
      : [];
    ...
    const head = [
      [...baseColsLeft, ...dayHeaders, ...baseColsRight, ...fixedColsRight],
    ];

    const body = rows.map((row) => {
      const cells: any[] = [
        { content: row.nome, styles: { fontStyle: "bold", fontSize: 7.5 } },
      ];
      ...
      weekDates.forEach((d) => {
        ...
        if (!hasData) {
          ...
          if (
            ...
            const shiftContent = day.shifts
              .map((s: any) => `${s.name}: R$ ${fmtShort(s.producaoReal)}`)
              .join("\n");
            const entContent = day.shifts.some((s: any) => s.entregas > 0)
              ? `\n${day.shifts
                  .filter((s: any) => s.entregas > 0)
                  .map((s: any) => `${s.name}:${s.entregas}`)
                  .join(" | ")}`
              : "";
            cells.push({
      ...
      if (isProducao) {
        cells.push(
          {
            content: fmtShort(row.totalDiaria),
            styles: { halign: "right", fontSize: 7.5 },
          },
          {
            content: fmtShort(row.totalTaxa),
            styles: { halign: "right", fontSize: 7.5 },
          },
        );
      } else {
        cells.push(
          {
            content: fmtShort(row.totalProducaoReal),
            styles: { halign: "right", fontSize: 7.5 },
          },
          {
            content: fmtShort(row.payoutTotal),
            styles: { halign: "right", fontSize: 7.5 },
          },
        );
      }
      ...
      if (includeTaxaCorridas) {
        cells.push({
          content: fmtShort(row.taxaCorridas),
          styles: { halign: "right", fontSize: 7.5 },
        });
      }
      cells.push(
        {
          content:
            row.adiantamentos > 0 ? `-${fmtShort(row.adiantamentos)}` : "—",
          styles: { halign: "right", fontSize: 7.5, textColor: [220, 38, 38] },
        },
        {
          content: fmtShort(row.totalLiquido),
          styles: {
            halign: "right",
            fontSize: 8,
            fontStyle: "bold",
            textColor: [EMERALD.r, EMERALD.g, EMERALD.b],
          },
        },
      );
    ...
    const totalRowCells: any[] = [
      { content: "TOTAL", styles: { fontStyle: "bold", fontSize: 7.5 } },
    ];
    weekDates.forEach((d) => {
      ...
      totalRowCells.push({
        content: daySum > 0 ? fmtShort(daySum) : "—",
    ...
    if (isProducao) {
      totalRowCells.push(
        {
          content: fmtShort(rows.reduce((s, r) => s + r.totalDiaria, 0)),
          styles: { halign: "right", fontStyle: "bold" },
        },
        {
          content: fmtShort(rows.reduce((s, r) => s + r.totalTaxa, 0)),
          styles: { halign: "right", fontStyle: "bold" },
        },
      );
    } else {
      totalRowCells.push(
        {
          content: fmtShort(rows.reduce((s, r) => s + r.totalProducaoReal, 0)),
          styles: { halign: "right", fontStyle: "bold" },
        },
        {
          content: fmtShort(rows.reduce((s, r) => s + r.payoutTotal, 0)),
          styles: { halign: "right", fontStyle: "bold" },
        },
      );
    }
    if (includeTaxaCorridas) {
      totalRowCells.push({
        content: fmtShort(rows.reduce((s, r) => s + r.taxaCorridas, 0)),
        styles: { halign: "right", fontStyle: "bold" },
      });
    }
    totalRowCells.push(
      {
        content: `-${fmtShort(rows.reduce((s, r) => s + r.adiantamentos, 0))}`,
        styles: {
          halign: "right",
          fontStyle: "bold",
          textColor: [220, 38, 38],
    ...
    doc.text(fmtBRL(effectiveTotalALiquidar), boxX + boxW - 4, finalY + 36, {
      align: "right",
    });
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.text(`Página ${i} de ${totalPages}`, pw - 8, ph - 4, {
        align: "right",
      });
    }
    ...
    alert(
      "Erro ao gerar o relatório PDF. Verifique se há dados válidos e tente novamente.",
    );
  }
}
